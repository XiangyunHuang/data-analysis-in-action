...
v <- prune_vocabulary(v, term_count_min = 10, term_count_max = 200, doc_proportion_max = 0.2)
# 词向量化
...
```{r}
lda_model$get_top_words(n = 3, topic_number = c(1L, 3L), lambda = 0.3)
```

5 个主题各自的 Top 词

```{r}
lda_model$get_top_words(n = 5, topic_number = c(1L, 5L), lambda = 0.3)
```

结果有点意思，说明益辉喜欢诗词歌赋、代码编程、回忆青春、做菜吃饭。
