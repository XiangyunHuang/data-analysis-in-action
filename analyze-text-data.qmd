...
中文环境没有英文的词干化（lemmatization）过程（比如 applied / applies -\> apply），只考虑分词、词性。[spacyr](https://github.com/quanteda/spacyr/) 包通过 reticulate 包调用 Python 社区的自然语言处理模块 [spacy](https://spacy.io) ，支持多种语言的分词、词性识别等。配置好 Python 环境，下载中文环境下的语言模型。
