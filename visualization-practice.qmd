...
::: {.callout-tip}
核密度估计方法在可视化中应用是相当广泛的，下面简单介绍 ggplot2 的核密度估计原理，做到知其然且知其所以然。上图是采用 `spatstat.explore::density.ppp()` 实现高斯核密度估计， [**SpatialKDE**](https://github.com/JanCaha/SpatialKDE) 包也可以。然后，在不规则的区域上插值，调用 `interp::interp()` 或 `akima::bilinear()` 在规则网格上插值，再调用 **ggplot2** 包函数 `geom_contour()` 和 `geom_contour_filled()` 实现可视化。一维、二维和多维核平滑方法的详细介绍见参考书籍 @Chacon2018 和 @Richard2022 。
:::
...
<!-- 
均值回归现象最早是高尔顿在甜豌豆实验中发现的，实际上，均值回归现象在社会经济和自然界中广泛存在，比如一个人的智力水平受家族平均水平的影响。
-->
...
<!-- 
高维列联表，联合独立性检验，条件独立性检验，耶茨连续矫正是什么意思
多维列联表
...
m
## Model of joint independence of Victim from Death and Defend
# 联合独立性 (Victim, Death)  (Victim, Defend)  (Death, Defend)
fm <- loglin(m, list(c(1, 2), c(1, 3), c(2, 3)))
fm 
## 拟合对数线性模型
fm <- loglin(m, list(c(1), c(2), c(3)))
fm 
# lrt 似然比 the Likelihood Ratio Test statistic.
# pearson 皮尔逊  the Pearson test statistic (X-squared).
...
计数数据转逻辑回归模型，二分类（死刑与否）

```{r}
# 长变宽 
ethnicity2 <- reshape(
  data = ethnicity, direction = "wide", idvar = c("Victim", "Defend"),
  timevar = "Death", v.names = "Freq", sep = "_"
)

fit_dvp2 <- glm(cbind(Freq_No, Freq_Yes) ~., data = ethnicity2, family = binomial(link = "logit"))
```
...
```{r}
# Pearson X^2 统计量
sum(residuals(fit_dvp2, type = "pearson")^2)
```
...
泰坦尼克号处女航乘客数量按船舱、性别、年龄和存活情况分层，用 [**ggalluvial**](https://github.com/corybrunson/ggalluvial/) 包[@Brunson2020]绘制桑基图展示多维分类数据。
...
百分比堆积柱形图展示多维分类数据 [**ggstats**](https://github.com/larmarange/ggstats/)

<!-- 
ggstats 提供的图层 stat_prop 是 stat_count 的变种
as.data.frame(Titanic) 中 Age 一列会自动聚合吗？
by = Class 按 Class 分组聚合，统计 Survived 的比例，提供 prop 计算的变量，传递给 geom_text 以添加注释，position 设置将注释放在柱子的中间
-->
