...
顺便一提，类似的用于概率推理和统计分析的框架，还有 Python 社区的 [PyMC](https://github.com/pymc-devs/pymc) 和 [TensorFlow Probability](https://github.com/tensorflow/probability)，它们采用的 MCMC 采样算法也是基于 NUTS 的 HMC 算法。
...
Horseshoe 先验（Horse shoe）[@Piironen2017] 指定回归系数 $\alpha,\bm{\beta}$ 的先验分布如下
...
::: callout-note
**brms** 包（版本 2.20.1）在模型中全局性地使用 horseshoe 先验和 R2D2 先验信息。
:::
...
1.  在 @sec-choose-prior 的基础上，比较 Stan 实现的贝叶斯 Lasso 和 R 包 **glmnet** 的结果。发现 **glmnet** 包是很有竞争力的，收缩效果比 Stan 还好，运行速度很快，但 Stan 的优势在于不限于 **glmnet** 包支持的常见分布族，如高斯、二项、泊松、多项、Cox 等。本质上，这是 Stan 作为一门编程语言的优势，只要知道概率分布的数学表达式，总是可以用 Stan 编码出来的。
