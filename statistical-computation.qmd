...
模拟数据矩阵 X 与上述记号 $X^{\top}$ 是对应的，记号 $x_i^{\top}$ 表示数据矩阵的第 $i$ 行。$\alpha$ 是逻辑回归方程的截距，$\beta$ 是 $k$ 维向量，$X$ 是 $n \times k$ 维的矩阵且 $n > k$，$y$ 是 $n$ 维向量。极大化对数似然函数 @eq-log-logit-lik ，就是求解一个多维非线性无约束优化问题。方便起见，将 $\alpha$ 合并进 $\beta$ 向量，另 函数 `optim()` 默认求极小，因此在对数似然函数前添加负号。
