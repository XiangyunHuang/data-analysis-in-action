...
&= \sum_{i=1}^{n}\Big[(2 y_i - 1) (\alpha + x_i^{\top}\boldsymbol{\beta}) - y_i \log(1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}) \Big]
\end{aligned}
$$ {#eq-log-logit-lik}
...
\frac{\partial \ell(\alpha,\boldsymbol{\beta})}{\partial \beta} &= \sum_{i=1}^{n}x_i\Big[(2y_i - 1) - y_i \frac{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}{1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}} \Big]
\end{aligned}
...
log_logit_lik_grad <- function(alpha, beta, X = X, y = y) {
  ...
  (2 * sum(y) - n) - exp(alpha + sum(t(X) %*% beta)) / (1 + exp(alpha + sum(t(X) %*% beta))) * sum(y) * cbind(1, X)
}
```
