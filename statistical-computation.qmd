...
其中，$X \in \mathbb{R}^{m\times n}$， $\bm{y} \in \mathbb{R}^m$，$\beta \in \mathbb{R}^n$， $0 < \lambda \in \mathbb{R}$ 。
...
记 $X = (\bm{x}_1, \bm{x}_2, \cdots, \bm{x}_n)$ 是一个$n \times k$矩阵，则 $\bm{x}_i$ 表示数据矩阵 $X$ 的第 $i$ 行，一共有 $k$ 列。同理 $\boldsymbol{\beta} = (\beta_1, \beta_2, \cdots, \beta_k)$ ，对第 $i$ 次观测
...
::: {.callout-tip}
对于常见的统计模型，**brms** 包[@brms2017]都内置了预编译的 Stan 程序，下面用 **brms** 包的函数 `brm()` 拟合泊松广义线性模型，结果和 Base R 内置的 函数 `glm()` 拟合结果几乎一致，因编译和抽样的过程比较花费时间，速度不及 Base R。
...
以上是简单的泊松型广义线性模型，下面增加一点复杂度，模拟带漂移项的泊松型广义线性混合效应模型。

```{r}
set.seed(2023)
n <- 2500 # 样本量
beta <- c(0.5, 0.3, 0.2)
X <- MASS::mvrnorm(100 , mu = rep(0, 2), Sigma = matrix(c(1, 0.8, 0.8, 1), 2))
u <- rep(c(2, 4), each = n / 2)
g <- factor(rep(1:25, each = 100)) # 25 个组 # 每个组 100 个观察值
lambda <- u * exp(rep(cbind(1, X) %*% beta, each = 25) )
y <- rpois(n, lambda = lambda)

simdata <- data.frame(y, X, u, g)
colnames(simdata) <- c("y", "x1", "x2", "u", "g")
...
下面继续增加复杂度，随机效应中含随机过程的泊松型广义线性混合效应模型。

设定参数，真实、模拟 重构、Stan 重构的比较
