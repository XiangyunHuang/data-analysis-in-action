...
从一个逻辑回归模型模拟一组样本，2500 条记录，10 个观测变量，其中只有变量 $X_1$ 和 $X_2$ 的系数非零，参数设定为 $\alpha = 1,\beta_1=3,\beta_2=-2,\beta_i = 0,i=3,\cdots,10$，模拟数据的代码如下：
...
plot(fit_glm$lambda, ylab = expression(lambda))
```
