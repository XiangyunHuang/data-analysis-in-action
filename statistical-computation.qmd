...
$$
\mathrm{E}Y = p = \mathrm{Logistic}(X\boldsymbol{\beta}) = \frac{1}{1 + e^{-(\alpha + X\boldsymbol{\beta})}}
$$
...
L(\alpha,\boldsymbol{\beta}) &= \prod_{i=1}^{n} p_i^{y_i}(1 - p_i)^{1 - y_i} \\ 
     ...
     &= \Big(\frac{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}{1 + e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}\Big)^{\sum_{i=1}^{n}y_i}\Big(\frac{1}{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}\Big)^{n - \sum_{i=1}^{n}y_i} \\
\end{aligned}
$$
...
&= \sum_{i=1}^{n} y_i \log \Big(\frac{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}{1 + e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}\Big) + (n - \sum_{i=1}^{n}y_i) \log\Big(\frac{1}{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}\Big) \\
&= (2 \sum_{i=1}^{n} y_i - n)  \Big(\alpha + \sum_{j=1}^{k}x_j\beta_j\Big) - \sum_{i=1}^{n} y_i \log\Big(1 + e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}\Big)
\end{aligned}
$$ {#eq-log-logit-lik}
...
$$
\begin{aligned}
\frac{\partial \ell(\alpha,\boldsymbol{\beta})}{\partial \alpha}  &= (2\sum_{i}^{n}y_i -n) - \frac{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}{1 + e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}} \sum_{i}^{n}y_i \\
\frac{\partial \ell(\alpha,\boldsymbol{\beta})}{\partial \beta_j} &= x_j (2\sum_{i}^{n}y_i -n) - x_j  \frac{e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}}{1 + e^{\alpha + \sum_{j=1}^{k}x_j\beta_j}} \sum_{i}^{n}y_i
\end{aligned}
$$ {#eq-log-logit-lik-partial}

其中， $\sum_{i=1}^{n} y_i$ 表示 $y_i = 1$ 的观测值数量，$(n - \sum_{i=1}^{n}y_i)$ 表示 $y_i = 0$ 的观测值数量， $j = 1,\cdots,k$ ，通过 IWLS （迭代加权最小二乘）可以求解此优化问题。
...
log_logit_lik <- function(alpha, beta, X = X, y = y) {
  (2 * sum(y) - length(y)) * (alpha + sum(X * beta)) -
    sum(y) * log(1 + exp(alpha + sum(X * beta)))
}
log_logit_lik_grad <- function(alpha, beta, X = X, y = y){
  c(
    (2 * sum(y) - length(y)) - exp(alpha + sum(X * beta)) / (1 + exp(alpha + sum(X * beta))) * sum(y)
  )
}
```
