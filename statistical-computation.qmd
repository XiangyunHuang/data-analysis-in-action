...
$$
\mathrm{E}Y = p = \mathrm{Logistic}(X\boldsymbol{\beta}) = \frac{1}{1 + e^{-(\alpha + X^{\top}\boldsymbol{\beta})}}
$$
...
记 $X^{\top} = (x_1, x_2, \cdots, x_n)^{\top}$ ，则 $x_i^{\top}$ 表示数据矩阵 $X^{\top}$ 的第 $i$ 行，一共有 $k$ 列。同理 $\boldsymbol{\beta} = (\beta_1, \beta_2, \cdots, \beta_k)$ ，其中 $j = 1,\cdots,k$ ，对第 $i$ 次观测
...
L(\alpha,\boldsymbol{\beta}) &= \prod_{i=1}^{n} p_i^{y_i}(1 - p_i)^{1 - y_i} \\ 
     &= p_i^{\sum_{i=1}^{n} y_i}(1 - p_i)^{n - \sum_{i=1}^{n} y_i} \\ 
     &= \Big(\frac{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}{1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}\Big)^{\sum_{i=1}^{n}y_i}\Big(\frac{1}{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}\Big)^{n - \sum_{i=1}^{n}y_i} \\
\end{aligned}
$$
...
&= \sum_{i=1}^{n} y_i \log \Big(\frac{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}{1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}\Big) + (n - \sum_{i=1}^{n}y_i) \log\Big(\frac{1}{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}\Big) \\
&= (2 \sum_{i=1}^{n} y_i - n)  \Big(\alpha + x_i^{\top}\boldsymbol{\beta}\Big) - \sum_{i=1}^{n} y_i \log\Big(1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}\Big)
\end{aligned}
$$ {#eq-log-logit-lik}
...
$$
\begin{aligned}
\frac{\partial \ell(\alpha,\boldsymbol{\beta})}{\partial \alpha}  &= (2\sum_{i}^{n}y_i -n) - \frac{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}{1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}} \sum_{i}^{n}y_i \\
\frac{\partial \ell(\alpha,\boldsymbol{\beta})}{\partial \beta_j} &= x_j (2\sum_{i}^{n}y_i -n) - x_j  \frac{e^{\alpha + x_i^{\top}\boldsymbol{\beta}}}{1 + e^{\alpha + x_i^{\top}\boldsymbol{\beta}}} \sum_{i}^{n}y_i
\end{aligned}
$$ {#eq-log-logit-lik-partial}

其中， $\sum_{i=1}^{n} y_i$ 表示 $y_i = 1$ 的观测值数量，$(n - \sum_{i=1}^{n}y_i)$ 表示 $y_i = 0$ 的观测值数量，通过 IWLS （迭代加权最小二乘）可以求解此优化问题。
...
# X 是 n x k 维的矩阵
# y 是 n 维向量
log_logit_lik <- function(alpha, beta, X = X, y = y) {
  (2 * sum(y) - length(y)) * (alpha + sum(t(X) %*% beta)) -
    sum(y) * log(1 + exp(alpha + sum(t(X) %*% beta)))
}
log_logit_lik_grad <- function(alpha, beta, X = X, y = y) {
  n <- length(y)
  k <- length(beta)
  # n > k
  (2 * sum(y) - n) - exp(alpha + sum(t(X) %*% beta)) / (1 + exp(alpha + sum(t(X) %*% beta))) * sum(y) * c(1, diag(X))
}
```

一般情况下，可以用 Base R 内置的函数 `optim()` 或 **nloptr** 包来求解。
