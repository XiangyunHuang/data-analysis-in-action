...
二战后，朗格拉普岛成为美国在太平洋的一个领地，1954 年，美国在该岛进行了氢弹核武器测试，核武器测试导致大量的核污染，严重破坏了当地的生态。几十年后，一批科学家登陆该岛研究生态恢复情况，研究当地居民重返家园的可行性，收集了一些数据。数据集 rongelap 记录了 157 个测量点的伽马射线强度，即在时间间隔 `time` 内放射的粒子数目 `counts`，测量站点的横纵坐标分别为 `cX` 和 `cY`。
...
xyplot(cY ~ cX,
  ...
  lattice.options = list(
    layout.widths = list(
      left.padding = list(x = -0.08, units = "inches"),
      right.padding = list(x = -0.2, units = "inches")
    ),
    layout.heights = list(
      bottom.padding = list(x = -0.2, units = "inches"),
      top.padding = list(x = -0.25, units = "inches")
    )
  ),
  # 设置坐标轴字体大小
  par.settings = list(
  ...
  panel = function(...) {
    ...
    panel.lines(rongelap_coastline$cX, rongelap_coastline$cY, col = "black")
  }
)
```

在朗格拉普岛上采样点的位置及观测到的辐射强度，见下图

```{r}
#| label: fig-rongelap-concentration
#| fig-cap: "采样点的放射强度在岛上的分布"
#| echo: false
#| fig-showtext: true
#| fig-width: 6
#| fig-height: 6
...
核污染是因为辐射元素衰变产生的，辐射代表单位时间释放出来的粒子数目，因此，建立泊松型广义线性模型分析数据。
