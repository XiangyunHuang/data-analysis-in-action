...
![点击注册](screenshots/git-github/sign-up.png){#fig-sign-up width="60%"}

接着，输入注册用的邮箱地址，比如 Outlook 和 Gmail 等。

![输入邮箱](screenshots/git-github/email.png){#fig-email width="60%"}

除了邮箱外，继续输入密码、用户名等，密码可以选用浏览器自动生成的复杂字符串，只要没有被别人占用，用户名可以按着自己的喜好填写。

![输入用户名](screenshots/git-github/username.png){#fig-username width="60%"}

接着，系统要验证来注册 Github 账户的人是否是真人。

![回答问题](screenshots/git-github/verify-account.png){#fig-verify-account width="60%"}

正确回答界面上出现的问题后，进入下一步，系统会给你之前提供的邮箱发送一个验证码。

![输入验证码](screenshots/git-github/verify-code.png){#fig-verify-code width="60%"}

将收到的验证码输入进去，完成账户验证。

![验证账户](screenshots/git-github/create-account.png){#fig-create-account width="60%"}

创建账户后，将自动进入如下界面，接下来，可以创建代码仓库了。

![创建代码仓库](screenshots/git-github/git-repo.png){#fig-git-repo width="60%"}
