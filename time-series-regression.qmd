...
air_passengers_ts <- ts(mod$fitted.values, start = c(1949, 1), frequency = 12)
plot(AirPassengers)
lines(air_passengers_ts, col = "red")
```

从拟合结果来看，存在欠拟合，年周期内波动幅度随时间有变化趋势，趋势和周期存在交互作用

```{r}
mod2 <- gam(y ~ s(year, month),
  data = air_passengers_tbl, family = gaussian
)
summary(mod2)
```
...
vis.gam(mod2, theta = -35, phi = 20, ticktype = "detailed", expand = .65)
```
