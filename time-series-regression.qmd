...
ggplot(data = meituan_log_return) +
...
acf(meituan_log_return, main = "对数收益率的自相关图")
```
...
library(fGarch)
meituan_garch <- garchFit(~ 1 + garch(1, 1), data = meituan_log_return, trace = FALSE)
summary(meituan_garch)
```

函数 `garchFit()` 的参数 `cond.dist` 默认值为 `"norm"` 表示正态分布。
