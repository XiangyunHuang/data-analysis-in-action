...
train_pred_ts <- ts(data = train_pred, start = c(1949, 4), frequency = 12)
test_pred_ts <- ts(data = test_pred, start = c(1957, 2), frequency = 12)
plot(AirPassengers)
...
evaluate(model, test_data[, c("x1", "x2", "x3")], test_data[, "y"])
# 测试集上的预测
mod_mlp_pred <- predict(model, test_data[, c("x1", "x2", "x3")]) 
sqrt(mean((test_data[, "y"] - mod_mlp_pred)^2)) # 计算均方根误差
```
