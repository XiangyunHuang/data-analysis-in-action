...
下面简化这个网络，仅考虑贡献者也是维护者的情况，就是说网络中所有节点即是维护者也是贡献者，这会过滤掉组织机构、大量没有在 CRAN 发过 R 包的贡献者、从没给其它维护者做贡献的维护者。
...
节点的大小以该维护者维护的 R 包数量来表示，边的大小以维护者之间协作次数来表示。为了方便可视化，仅保留协作次数大于 1 的边。
...
下面考虑用 R 包可视化这个复杂的有向带权网络。`pdb_authors_net_edge` 和 `pdb_authors_net_vertex` 都是数据框，首先调用 igraph 包将其转化为网络类型 `igraph` ，然后绘制网络图。
...
plot(pdb_authors_graph,
  ...
  vertex.label.cex = (V(pdb_authors_graph)$vertex_cnt) / 20
)
```
