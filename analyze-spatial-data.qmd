...
当 $u$ 趋于 0，则 $\rho(u; \phi, \tau_{rel}^2 ) = \tau_{rel}^2$ ，另外， $\phi$ 只能取正值，$\tau_{rel}^2$ 的取值介于 0-1 之间，**nlme** 包的默认设置下，$\phi$ 的初始值为 $0.9 \times \min_{i,j \in A} u_{ij}$，即所有点之间距离的最小值的 90%，而 $\tau_{rel}^2$ 为 0.1 。
