...
**nlme** 包的函数 `corExp()` 表示自相关结构是指数型的，为了方便估计参数做了一些重参数化的操作，带块金效应的方差协方差结构为
...
当 $u$ 趋于 0，则 $\rho(u; \phi, \tau_{rel}^2 ) = \tau_{rel}^2$ ，另外， $\phi$ 只能取正值，$\tau_{rel}^2$ 的取值介于 0-1 之间，**nlme** 包的默认设置下，$\phi$ 的初始值为 $0.9 \times \min_{i,j \in A} u_{ij}$，即所有点之间距离的最小值的 90%，而 $\tau_{rel}^2$ 为 0.1 。**nlme** 包的函数 `gls()` 实现限制极大似然估计方法，可以拟合存在异方差的一般线性模型，也是一种特殊的带自相关结构的线性混合效应模型，随机效应的结构可以看作异方差的结构。
...
根据前面的关系式，可以得到 $\tau^2$ 和 $\sigma^2$ 的估计。
...
接下来检查空间相关性， rongelap 数据集包含 157 个采样点的坐标，两两配对，共有 $(157 - 1) \times (157 - 2) / 2 = 12090$ 对。拟合残差的半变差函数
...
结合拟合残差的经验半变差 @fig-rongelap-vario ，说明添加指数型空间自相关性是合理的。
