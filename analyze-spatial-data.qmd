...
[**glmmTMB**](https://github.com/glmmTMB/glmmTMB) 包[@Brooks2017]可以拟合空间广义线性混合效应模型，使用语法和前面介绍的 **lme4** 包一样。[**glmmTMB**](https://github.com/glmmTMB) 基于 [**TMB**](https://github.com/kaskr/adcomp) 框架可以拟合线性和广义线性混合效应模型及其扩展，包括零过多（零膨胀）zero-inflation 模型，估计方法是极大似然，实现工具是 TMB。随机效应假定是高斯分布，拉普拉斯方法近似随机效应的高维积分，自动微分的方式计算梯度。
...
**sf** 包提供了大量空间操作函数，比如用 `st_bbox()` 获得东西南北的边界点。

```{r}
st_bbox(rongelap_coastline_sf)
```

连接点 POINT 构造多边形 POLYGON 对象。

```{r}
rongelap_coastline_sfp <- st_cast(st_combine(st_geometry(rongelap_coastline_sf)), "POLYGON")
```

@fig-point-to-polygon 上下两个子图分别展示点和线。上图是原始的点数据，下图是以点带线，串联 POINT 构造 POLYGON 后的多边形。
...
为了确保覆盖整个岛，还需要一点缓冲空间，就是说在给定的边界线外围再画一个多边形，这可以用函数 `st_buffer()` 根据海岸线构造 POLYGON 的缓冲区，缓冲距离选择 50 米。
...
接下来，利用函数 `st_make_grid()` 根据海岸缓冲线的边界打网格，朗格拉普到是狭长的，因此，网格是 $25\times 50 $ 的，意味着水平方向 25 行，垂直方向 50 列，细密程度是可以调整的，越密岛内的格点越多，核污染浓度计算越精确，同时也越耗时。

```{r}
rongelap_coastline_grid <- st_make_grid(rongelap_coastline_buffer, n = c(50, 25))
```
...
然后，去掉和岛屿没有交集的网格，留下缓冲区以内的网格。**sf** 包的函数 `st_intersection()` 可以将两个多边形相交的部分取出来，全岛网格化后，如 @fig-rongelap-grid 所示。

```{r}
rongelap_grid <- st_intersection(rongelap_coastline_grid, rongelap_coastline_buffer)
```
...
全岛一共划分为 268 个小网格，网格的中心点作为新的采样点，在这些新的采样点上预测核污染浓度。利用函数 `st_centroid()` 可以获取网格的中心位置，下 @fig-rongelap-centroid 新的采样点在全岛的分布。

```{r}
#| label: fig-rongelap-centroid
#| fig-cap: "朗格拉普岛的采样点"
#| fig-width: 7
#| fig-height: 4
#| fig-showtext: true
...
ggplot() +
  geom_sf(data = rongelap_grid, fill = NA, color = "gray30") +
  geom_sf(data = st_centroid(rongelap_grid)) +
  theme_void()
```
