...
**nlme** 包的函数 `gls()` 的名称 `gls` 是 Generalized Least Squares 广义最小二乘的意思。顾名思义，广义最小二乘是最小二乘的推广形式。当模型残差不满足独立同分布的条件，存在相关性或异方差，就需要广义最小二乘 （Generalized Least Squares ，简称 GLS）。在前面的第 @sec-rongelap-slmm 小节，广义最小二乘估计模型 @eq-rongelap-gaussian-slmm 的参数，下面继续介绍其原理。线性模型的一般形式如下：
...
现在的目标是找到一个 $\hat{\bm{\beta}}$ 使得 $\mathsf{Var}\{\bm{y} - X\hat{\bm{\beta}}\}$ 最小。

类似地，对于一次新的观测 $\bm{x}_0$，它是一个 $1 \times p$ 的行向量，现在的目标是找到一个 $\hat{y}_0$ ，在满足无偏性的条件下，使得预测的均方误差 $\mathsf{E}\{(\hat{y}_0 - y_0)^2\}$ 最小，也就是预测方差 $\mathsf{Var}\{\hat{y}_0 - y_0\}$ 最小。不妨设 $\hat{y}_0$ 是关于 $\bm{y}$ 的线性组合，即 $\hat{y}_0 = \bm{w}^{\top}\bm{y}$ ，此处，$\bm{w}$ 是 $N \times 1$ 维的权重向量。找 $\hat{y}_0$ 的问题转为求解如下最优化问题。
...
$$
\mathsf{E}\{\hat{y}_0 - y_0\} = \mathsf{E}\{\bm{w}^{\top}X\bm{\beta} - \bm{x}_0\bm{\beta}\} = \bm{w}^{\top}X\bm{\beta} - \bm{x}_0\bm{\beta} = 0
$$
...
$$
\begin{aligned}
\mathsf{Var}\{\hat{y}_0 - y_0\} &= \mathsf{Var}\{\bm{w}^{\top}\bm{y} - \bm{x}_0\bm{\beta}- \epsilon_0\} \\
&= \mathsf{Var}\{\bm{w}^{\top}(X\bm{\beta} + \bm{\epsilon}) - \bm{x}_0\bm{\beta} - \epsilon_0\} \\
...
& = \bm{w}^{\top}\mathsf{Cov}\{\bm{\epsilon},\bm{\epsilon}^{\top}\} \bm{w} - \bm{w}^{\top} \mathsf{Cov}\{\bm{\epsilon}, \epsilon_0^{\top}\} - \mathsf{Cov}\{\epsilon_0, \bm{\epsilon}^{\top}\} \bm{w} + \mathsf{Cov}\{\epsilon_0, \epsilon_0 \} \\
...
令$$
\Sigma_{11} = \mathsf{Cov}\{\bm{\epsilon},\bm{\epsilon}^{\top}\}, \Sigma_{01} = \mathsf{Cov}\{\epsilon_0, \bm{\epsilon}^{\top}\}, \Sigma_{00} = \mathsf{Cov}\{\epsilon_0, \epsilon_0 \}
$$

采用拉格朗日乘数法将约束优化问题 @eq-lmm-optim 转为如下无约束优化问题

$$
\min_{\bm{w},\kappa} \quad \bm{w}^{\top}\Sigma_{11} \bm{w} - 2 \Sigma_{01} \bm{w} + \Sigma_{00} + \kappa (\bm{w}^{\top}X - \bm{x}_0)
$$

对 $\bm{w},\kappa$ 分别求偏导，并令偏导等于 0，可得

$$
\begin{aligned}
2 \bm{w}^{\top} \Sigma_{11} - 2 \Sigma_{10} + \kappa X^{\top} = \bm{0} \\
\bm{w}^{\top}  X - \bm{x}_0 = \bm{0}
\end{aligned}
$$

第一个式子右乘 $\Sigma_{11}^{-1}X$ ，再将第二个式子代入第一个式子，有

$$
2 \underbrace{\bm{w}^{\top} X}_{\bm{x_0}} -2 \Sigma_{10}\Sigma_{11}^{-1}X + \kappa X^{\top}\Sigma_{11}^{-1}X = 0
$$

可得

$$
\kappa = 2 (X^{\top}\Sigma_{11}^{-1}X)^{-1} (\bm{x_0} - \Sigma_{10}\Sigma_{11}^{-1}X)
$$

将 $\kappa$ 代入第一个式子，可得

$$
\bm{w}^{\top} = (\Sigma_{10} - (X^{\top}\Sigma_{11}^{-1}X)^{-1}(\bm{x}_0 - \Sigma_{10}\Sigma_{11}^{-1}X)) \Sigma_{11}^{-1}.
$$
...
&= (\Sigma_{10} - (X^{\top}\Sigma_{11}^{-1}X)^{-1}(\bm{x}_0 - \Sigma_{10}\Sigma_{11}^{-1}X)) \Sigma_{11}^{-1}\bm{y} \\
&= \Sigma_{10} \Sigma_{11}^{-1}\bm{y} - (X^{\top}\Sigma_{11}^{-1}X)^{-1}(\bm{x}_0 - \Sigma_{10}\Sigma_{11}^{-1}X)\Sigma_{11}^{-1}\bm{y}
\end{aligned}
$$
...
&= \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\sum_{i=1}^{N}w_i \mathsf{Cov}\{\lambda(x_i),\lambda(x_0)\} \\
& = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\bm{w}^{\top} \mathsf{Cov}\{\bm{\lambda},\lambda(x_0)\} \\
& = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\bm{w}^{\top} \bm{u}.
\end{aligned}
$$
...
$$
\min_{\bm{w},\kappa} \mathcal{L}(\bm{w},k) = \frac{1}{2}\bm{w}^{\top}V\bm{w} + \frac{1}{2}\sigma_{0}^2 - \bm{w}^{\top} \bm{u} + \kappa (\bm{w}^{\top} \bm{1} - 1)
$$
...
$$
\begin{aligned}
\frac{\partial \mathcal{L}(\bm{w},\kappa)}{\partial \bm{w}} &=  \bm{w}^{\top} V -  \bm{u}^{\top} + \kappa \bm{1}^{\top}= \bm{0} \\
\frac{\partial \mathcal{L}(\bm{w},k)}{\partial \kappa} &= \bm{w}^{\top} \bm{1} - 1 = 0
\end{aligned}
$$
...
所以，$\bm{w}^{\top} = \bm{u}^{\top}V^{-1}$ ，在位置 $x_0$ 处的预测 $\hat{\lambda}(x_0) = \bm{u}^{\top}V^{-1}\bm{\lambda}$ ，预测的均方误差 $\mathsf{E}\{[\hat{\lambda}(x_0) - \lambda(x_0)]^2\} = \mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} = \sigma_0^2 - \bm{u}^{\top}V^{-1}\bm{u}$ 。根据数据可以估计经验半变差函数 @eq-semi-variogram 的参数，从而 $V$ 、 $\sigma_{0}^2$ 和 $\bm{u}$ 都是已知的。以 $V + \tau^2\mathsf{I}$ 替换 $V$ ，$\bm{\lambda} - \beta \bm{1}$ 替换 $\bm{\lambda}$ ，可得普通克里金插值的预测值及其均方误差。

$$
\begin{aligned}
\hat{\lambda}(x_0) &= \beta + \bm{u}^{\top}(V + \tau^2\mathsf{I})^{-1}(\bm{\lambda} - \beta \bm{1}) \\
