...
ggplot(data = mesh_matern, aes(x = x, y = fv)) +
  ...
  scale_color_viridis_d(labels = expression(nu == 0.5, nu == 1.5, nu == 2.5), option = "C") +
...
从输出结果来看，模型固定效应的截距项 $\beta$ 为 `1.831`，随机效应的方差 $\sigma^2$ 为 `0.2964`，自协方差函数中参数 $\rho = 0.00968$，对比函数 `Matern()` 实现的指数型自相关函数公式与 @eq-matern-formula ，输出结果需要转化一下，以符合 @eq-matern-formula ，则 $\phi = 1 / 0.00968 = 103.3$ ，即空间相关性的最大影响距离为 103.3 米。
...
rongelap_grid_df2 <- reshape(
  ...
  varying = c("pred_sp", "pred_sk"), 
  times = c("spaMM", "nlme"),
  v.names = "pred", 
  timevar = "method",
  idvar = c("cX", "cY"),
  new.row.names = 1:(2 * 1612), 
  direction = "long"
)
...
ggplot(data = rongelap_grid_sf2) +
  geom_sf(aes(color = pred), size = 0.75) +
  scale_color_viridis_c(option = "C") +
