...
cloud(counts / time ~ cX * cY,
  ...
  aspect = c(0.75, 0.65),
  xlab = list("横坐标（米）", rot = 10),
  ylab = list("纵坐标（米）", rot = -80),
...
**nlme** 包的函数 `gls()` 的名称 gls 是 Generalized Least Squares 广义最小二乘的意思。顾名思义，广义最小二乘是最小二乘的推广形式。当模型残差不满足独立和同分布的条件，存在相关性或异方差，就需要广义最小二乘。

-   最小二乘

-   广义最小二乘 （Generalized Least Squares ，简称 GLS）
