...
随机过程 $S(d)$ 的自协方差函数常用的有指数型、幂二次指数型和 Matérn 型，形式如下：
...
理解函数 `corExp()` 的参数，设定 $\phi = 1, \tau_{rel}^2 = 0.2$，则由距离矩阵和相关函数构造的方差协方差矩阵如下：
...
**nlme** 包的函数 `corExp()` 表示自相关结构是指数型的，带块金效应的方差-协方差结构为
...
\begin{aligned}
\tau_{rel}^2   &= 0.6537518 \\
\sigma_{tol}^2 &= 2.777964^2 = 7.717084 \\
\tau^2   &= 0.6537518 \times 7.717084 = 5.045058 \\
\sigma^2 &= 2.777964^2 - 0.6537518 \times 7.717084 = 2.672026 \\
\phi  &= 454.9667323 \\
\beta &= 7.169426
\end{aligned}
$$
...
fit_rongelap_lme <- nlme::lme(
  counts / time ~ 1, data = rongelap, random = ~ 1 | dummary,
  correlation = nlme::corExp(
    value = c(200, 0.1), form = ~ cX + cY, nugget = TRUE
  )
)
...
**spaMM** 包的使用语法和前面介绍的 **nlme** 包类似，它可以拟合空间广义线性混合效应模型，拟合函数 `fitme()` 提供统一的使用语法，输出一个 **spaMM** 包内建的 HLfit 类型的数据对象。
...
从输出结果来看，模型固定效应的截距项 1.824，随机效应的方差为 0.2973，自协方差函数中参数 $\nu = 0.3642$，对比 **spaMM** 包的函数 `Matern()` 实现的 Matérn 型自相关函数公式与 @eq-matern-formula ，输出结果需要转化一下，以符合 @eq-matern-formula ，则 $\rho = \sqrt{2 \times 0.364211759} / 0.0061486 = 138.81$ ，即空间相关性的最大影响距离为 138.8 米。
