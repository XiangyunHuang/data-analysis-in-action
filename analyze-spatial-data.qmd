...
在研究区域 $\mathcal{D}$ 采集一些数据，已知采样点 $x_1,x_2,\ldots,x_n$ 的辐射强度分别为 $\lambda(x_1),\lambda(x_2),\ldots,\lambda(x_n)$ 。现在的问题是对于未采样的位置 $x_0$，它的辐射强度 $\lambda(x_0)$ 是多少？在满足无偏性的条件下，$\hat{\lambda}(x_0)$ 是使预测方差最小的估计。
...
$$
\begin{aligned}
\min_{w_i} & \mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} \\
& \mathsf{E}\{\hat{\lambda}(x_0) - \lambda(x_0) \} = 0
\end{aligned}
$$
