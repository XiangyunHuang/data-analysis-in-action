...
$$
\mathsf{E}\{\hat{y}_0 - y_0\} = \mathsf{E}\{\bm{w}^{\top}(X\bm{\beta} + \bm{\epsilon}) - (\bm{x}_0\bm{\beta} + \bm{\epsilon}_0)\} = \bm{w}^{\top}X\bm{\beta} - \bm{x}_0\bm{\beta} = 0
$$
...
$$
\min_{\bm{w},\bm{\kappa}} \quad \bm{w}^{\top}\Sigma_{11} \bm{w} - 2 \Sigma_{01} \bm{w} + \Sigma_{00} + (\bm{x}_0 - \bm{w}^{\top}X) \bm{\kappa}
$$

其中，$\bm{\kappa}$ 是 $p \times 1$ 维的拉格朗日乘子向量，对 $\bm{w},\bm{\kappa}$ 分别求偏导，并令偏导等于 0，可得

$$
\begin{aligned}
2 \Sigma_{11} \bm{w} - 2 \Sigma_{10} - X\bm{\kappa}  = \bm{0} \\
X^{\top}\bm{w}  - \bm{x}_0^{\top} = \bm{0}
\end{aligned}
$$
...
$$
\bm{\kappa} = 2 (X^{\top}\Sigma_{11}^{-1}X)^{-1} (\bm{x_0}^{\top} - X^{\top}\Sigma_{11}^{-1}\Sigma_{10})
$$
...
+ X(X^{\top}\Sigma_{11}^{-1}X)^{-1}(\bm{x}_0^{\top} - X^{\top}\Sigma_{11}^{-1}\Sigma_{10})) .
$$
