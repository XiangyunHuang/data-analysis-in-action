...
数据集 gambia 中变量 `x` 和 `y` 分别为村庄的横、纵坐标，参考系是 UTM。下面将 UTM 坐标系转化为 WGS 84 坐标系，以便将疟疾观测数据和冈比亚地形数据绘制在一幅图上。
...
# 将 gmb_map 数据从 SpatVector 类型转为 MULTIPOLYGON 类型
gambia_borders <- st_as_sf(gmb_map)
gambia_borders <- st_transform(gambia_borders, crs = sp::CRS("+proj=utm +zone=28"))
```
