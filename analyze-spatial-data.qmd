...
$$
\begin{aligned}
\log(\bm{\lambda}) &= \bm{1}\beta \\
\bm{y} & \sim \mathrm{Poisson}(\bm{t}\bm{\lambda})
\end{aligned}
$$

其中，$\bm{\lambda}$ 表示核辐射强度，$\beta$ 表示未知的截距，$\bm{y}$ 表示观测到的粒子数目，$\bm{t}$ 表示相应的观测时间。R 软件内置的 stats 包有函数 `glm()` 可以拟合上述广义线性模型，代码如下。
...
当 $\bm{\epsilon}$ 之间存在空间相关性时，$V$ 含有参数 $\sigma^2,\phi$ ，若模型带块金效应，则还含有参数 $\tau^2$ 。可见，上式不足以估计模型的参数，且看后续 @sec-maximum-likelihood 。
...
第一个式子左乘 $X^{\top}V_{11}^{-1}$ ，再将第二个式子代入第一个式子，有
...
进而 $\mathsf{Var}\{\hat{y}_0\} = \mathrm{tr}[\bm{w}\bm{w}^{\top}V_{11}]$ 。
