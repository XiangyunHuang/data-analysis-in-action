...
library(ggplot2)
ggplot() +
  geom_point(data = rongelap, aes(x = cX, y = cY), size = 1) +
...
ggplot(data = rongelap) +
  geom_histogram(aes(x = counts / time),
...
既然异方差比较严重，给每个位置分配一个独立的方差是合理的，相当于添加随机效应。简单起见，假定异方差是相互独立的。

```{r}
#| message: false

library(spaMM)
fit_poisson_glmm <- fitme(
  formula = counts ~ 1 + (1 | dummy) + offset(log(time)),
  data = rongelap, family = poisson(link = "log")
)
summary(fit_poisson_glmm)
```
