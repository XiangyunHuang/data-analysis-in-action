...
$\phi$ 只能取正值，$\tau_{rel}^2$ 的取值介于 0-1 之间，nlme
包的默认设置下，$\phi$ 的初始值为 $0.9 \times \min_{i,j \in A} u_{ij}$，所有点之间距离的最小值的 90%，$\tau_{rel}^2$ 为 0.1
...
除了指数型，还可以用其它模型来拟合空间相关性，从标准化半变差 @fig-rongelap-vario-norm 来看，存在非均匀的波动，如果空间相关性提取得很充分，则标准化半变差图中的数据点应是毫无趋势的，拟合线也应该是一条直线。
