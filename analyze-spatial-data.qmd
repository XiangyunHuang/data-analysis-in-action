...
$$
\begin{aligned}
\min_{w_i} \quad & \mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} \\
...
$$
\min_{w_i} \mathcal{L}(\bm{w},k) = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\bm{w}^{\top} \bm{u} + \kappa (\bm{w}^{\top} \bm{1} - 1)
$$

其中，$\kappa$ 表示拉格朗日乘子，$\bm{1}$ 表示 $N \times 1$ 维的全 1 向量。分别对 $\kappa$ 和 $\bm{w}$ 求偏导，并令偏导等于 0 ，则有

$$
\begin{aligned}
\frac{\partial \mathcal{L}(\bm{w},\kappa)}{\partial \bm{w}} &= 2 \bm{w}^{\top} V - 2 \bm{u}^{\top} = 0 \\
