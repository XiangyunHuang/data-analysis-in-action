...
在空间分析中，克里金插值（Kriging Interpolation）是非常基础且重要的内容，具有很好的统计性质，是最佳线性无偏预测（Best Linear Unbiased Predictor）， @fig-kriging 是简单克里金插值的示意图，下面介绍插值的原理。
...
其中，$\kappa$ 表示拉格朗日乘子，$\bm{1}$ 表示 $N \times 1$ 维的全 1 向量。对 $\bm{w}$ 和 $\kappa$ 分别求偏导，并令其各自等于 0 ，则有
