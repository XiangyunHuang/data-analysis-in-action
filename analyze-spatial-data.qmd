...
# 统计各村庄的情况
gambia_village <- aggregate(data = gambia, cbind("total" = 1, pos, pred_prob, sigmasq = pred_prob * (1 - pred_prob)) ~ x + y + green + phc, FUN = sum)
gambia_village$residuals <- (gambia_village$pos - gambia_village$pred_prob) / sqrt(gambia_village$sigmasq)

# 从这残差图来看，空间相关性已经没有了
plot(gambia_village$residuals)
# 残差的空间分布
...
# 残差的变差图
fit_gambia_village_vario <- Variogram(fit_gambia_village, form = ~ x + y, data = gambia_village, resType = "normalized")
plot(fit_gambia_village_vario)
# 相关性最大距离限制 30 公里
fit_gambia_village_vario2 <- Variogram(fit_gambia_village, form = ~ x + y, data = gambia_village, resType = "normalized", maxDist = 30000)
plot(fit_gambia_village_vario2)

fit_gambia_village_vario3 <- Variogram(fit_gambia_village, form = ~ x + y, data = gambia_village, resType = "response")
plot(fit_gambia_village_vario3)
```
