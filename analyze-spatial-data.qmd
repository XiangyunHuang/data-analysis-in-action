...
截距 $\beta$ 相当于平均水平，$\lambda(x_i)$ 可理解为辐射强度，$S(x_{i})$ 表示位置 $x_i$ 处的空间效应，$S(x),x \in \mathbb{R}^2$ 是平稳空间高斯过程。且 $Z_i$ 之间相互独立同正态分布 $\mathcal{N}(0,\tau^2)$ ，$Z_i$ 表示与空间效应无关的块金效应，即非空间的随机效应，可以理解为测量误差、空间变差或背景辐射，这里 $i = 1,\ldots, 157$。
...
$$
\begin{aligned}
\mathrm{Cov}( S(x_i), S(x_j) ) &= \sigma^2 \exp\big( -\frac{\|x_i -x_j\|_{2}}{\phi} \big) \\
\mathrm{Cov}( S(x_i), S(x_j) ) &= \sigma^2 \exp\big( -\frac{\|x_i -x_j\|_{2}^{2}}{2l^2} \big) \\
\mathrm{Cov}( S(x_i), S(x_j) ) &= \sigma^2 \frac{2^{1 - \nu}}{\Gamma(\nu)}
...
ggplot(data = mesh_matern, aes(x = x, y = fv)) +
  ...
  scale_color_viridis_d(labels = expression(nu == 0.5, nu == 1.5, nu == 2.5)) +
...
下面以一个简单示例理解自相关函数 `corExp()` 的作用，令 $\phi = 1, \tau_{rel}^2 = 0.2$，则由距离矩阵和自相关函数构造的方差协方差矩阵如下：
...
**nlme** 包还有好些自相关函数，如常用的高斯型自相关函数 `corGaus()` ，它与函数 `corExp()` 的作用类似，示例如下：
...
**nlme** 包的函数 `corExp()` 表示自相关结构是指数型的，为了方便估计参数做了一些重参数化的操作。
...
当 $u$ 趋于 0 时， $\rho(u; \phi, \tau_{rel}^2 ) = \tau_{rel}^2$ 。另，$\phi$ 取正值，$\tau_{rel}^2$ 取值介于 0-1 之间，在默认设置下，$\phi$ 的初始值为 $0.9 \times \min_{i,j \in A} u_{ij}$，即所有点之间距离的最小值的 90%， $\tau_{rel}^2$ 为 0.1 。
...
**nlme** 包给出截距项 $\beta$ 、相对块金效应 $\tau_{rel}^2$ 、高斯过程范围参数 $\phi$ 和方差 $\sigma_{tol}^2$ 的估计，
...
$$ 令 $\gamma(u_{ij}) = \frac{1}{2}\mathsf{Var}\{T(x_i) - T(x_j)\}$ 表示半变差，这里 $u_{ij}$ 表示采样点 $x_i$ 与 $x_j$ 之间的距离。考虑到

$$
\gamma(u_{ij}) = \frac{1}{2}\mathsf{E}\big\{\big[T(x_i) - T(x_j)\big]^2\big\} = \tau^2 + \sigma^2\big(1-\rho(u_{ij})\big)
$$

上式第一个等号右侧期望可以用样本来计算，称之为样本半变差函数，第二个等号右侧为理论半变差函数。为了便于计算，将距离做一定划分，尽量使得各个距离区间的样本对数接近，此时，第 $i$ 个距离区间上样本半变差函数值的计算公式如下：

$$
\hat{\gamma}(h_i) = \frac{1}{2N(h_i)}\sum_{j=1}^{N(h_i)}(T(x_i)-T(x_i+h'))^2, \ \ h_{i,0} \le h' < h_{i,1}
$$

其中，$[h_{i,0},h_{i,1}]$ 表示第 $i$ 个距离区间，$N(h_i)$ 表示第 $i$ 个距离区间内所有配对样本点的对数，只要两个点之间的距离在这个区间内，就算是一对。$\hat{\gamma}(h_i)$ 代表第 $i$ 个距离区间上的样本半变差函数值。rongelap 数据集包含 157 个采样点的坐标，两两配对，共有 $(157 - 1) \times (157 - 2) / 2 = 12090$ 对。下面拟合关于残差的样本半变差函数：
...
结果显示，距离在 0-89.44272 米之间的坐标点有 510 对，样本半变差函数值为 0.07006716。距离在 89.44272-144.22205 米之间的坐标点有 601 对，样本半变差函数值为 0.12719889，依此类推。将距离和计算的样本半变差绘制出来，即得到经验半变差图，如 @fig-rongelap-vario 所示。刚开始，半变差很小，之后随距离增加而增大，直到达到一个平台。由于半变差反比于空间相关性的程度，随着距离增加，空间相关性减弱。这说明数据中确含有空间相关性，模型中添加指数型空间自相关结构是合理的。
...
简单的广义线性模型并没有考虑距离相关性，它认为各个观测点的数据是相互独立的。因此，考虑采用广义线性混合效应模型，在广义线性模型的基础上添加位置相关的随机效应，用以刻画未能直接观测到的潜在影响。根据 ${}^{137}\mathrm{Cs}$ 放出的伽马射线在 $N=157$ 站点不同时间间隔 $t(x_{i})$ 的辐射量，建立泊松型空间广义线性混合效应模型。

$$
\begin{aligned}
\log\{\lambda(x_i)\} & =  \beta + S(x_{i}) + Z_{i} \\
Y_{i} &\sim \mathrm{Poisson}(t(x_{i})\lambda(x_i))
\end{aligned}
$$ {#eq-rongelap-poisson-sglmmm}

模型中，放射粒子数$Y_{i}$作为响应变量服从均值为 $t(x_{i})\lambda(x_i)$ 的泊松分布，其它模型成分的说明同前。

**nlme** 包不能拟合空间广义线性混合效应模型， **spaMM** 包可以，它的使用语法与前面介绍的函数 `glm()` 、 **nlme** 包都类似，函数 `fitme()` 可以拟合从线性模型到广义线性混合效应模型的一大类模型，且使用统一的语法，输出一个 `HLfit` 类型的数据对象。 **spaMM** 包的函数 `Matern()` 实现了梅隆型自协方差函数，指数型和幂二次指数型是它的特例。当固定 $\nu = 0.5$ 时，梅隆型自相关函数 `Matern()` 的形式退化为 $\exp(-\rho d)$ ，其中 `d` 表示距离，相当于前面出现的 $u$，$\rho$ 相当于前面出现的 $1/\phi$ 。
...
作为对比，在上一节空间线性混合效应模型的基础上，下面采用简单克里金方法预测。
...
将预测结果以散点图的形式呈现到图上，见下 @fig-rongelap-pred ，由于散点非常多，紧挨在一起就连成片了。上子图是 **spaMM** 包预测的结果，下子图是 **nlme** 包预测的结果，后者图像看起来会稍微平滑一些。由于从空间线性混合效应模型到空间广义线性混合效应模型的效果提升不多，在曲折处，图像的平滑差异才会比较明显。
...
plot(rongelap_grid_sf[c("pred_sp", "pred_sk")], pch = 16, cex = 0.75,
     breaks = (0:78) / 6, key.pos = 4, pal = hcl.colors)
```

考虑到核辐射在全岛的分布应当是连续性的，空间连续性也是这类模型的假设，接下来要绘制热力图。为了绘制热力图，先用 **stars** 包[@stars2022]将预测数据按原网格化的精度转化成栅格对象，裁减超出朗格拉普岛海岸线以外的内容。
...
除了矢量栅格化函数 `st_rasterize()` 和栅格剪裁函数 `st_crop()` ，**stars** 包还提供栅格数据图层 `geom_stars()`，这可以和 **ggplot2** 内置的图层搭配使用。下 @fig-rongelap-pred-sp 是 **ggplot2** 包和 **grid** 包一起绘制的辐射强度的热力分布图。图左侧一小一大两个虚线框是放大前后的部分区域，展示朗格拉普岛核辐射强度的局部变化。
