...
空间分析的内容十分广泛，本章仅以一个模型和两个数据简略介绍。

空间广义线性混合效应模型在统计中具有重要的地位，在地质统计、卫生统计、气象统计和空间计量等领域有广泛的应用，如分析地区范围内的疟疾分布，有限气象站点条件下，预测地区 PM2.5 污染物浓度分布等。
...
冈比亚国家发起基础健康服务（Primary Health Care，简称 PHC）项目 ，在 2-3 年内建立覆盖全国各个村庄的服务站点，向村民们发放蚊帐，而后，冈比亚国家蚊帐杀虫项目对各村庄展开调查，收集蚊帐发放、使用情况，对发放的蚊帐杀虫，根据儿童血液样本中是否含有疟原虫确定是否感染疟疾，调查疟疾的流行度，调查在 1992 年雨季前完成，记录了 65 个村庄 5 岁以下儿童是否感染疟疾的情况，一共收集了 2035 个样本。冈比亚是非洲西部的一个贫穷小国家，国内冈比亚河横穿全国，流入大西洋，当地属热带气候，水网密布。下面借助[**geodata** 包](https://github.com/rspatial/geodata)包获取[SRTM](https://srtm.csi.cgiar.org/)（Shuttle Radar Topography Mission，简称 SRTM）和 [GADM](https://gadm.org/)（Database of Global Administrative Areas，简称 GADM）开放的地形和行政区划数据，绘制冈比亚地图。SRTM 对纬度范围在南纬 60 至北纬 60 度的地区，提供 90 米精度的海拔数据，GADM 开放全球各国的各级行政区划数据。**geodata** 获取的冈比亚海拔数据，返回 SpatRaster 类型的 **terra** 数据对象，它是一个栅格数据。**geodata** 获取的冈比亚二级级行政区数据，返回 SpatVector 类型的 **terra** 数据对象，它是一个多边形边界地图数据。**geodata** 包下载数据的代码如下：
