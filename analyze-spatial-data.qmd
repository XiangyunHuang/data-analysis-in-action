...
现在主要关注上式右侧中包含的二次型，将 $\hat{\bm{\beta}}(\bm{\theta})$ 代入，可得

$$
\log \mathcal{L}(\bm{\theta}) = -\frac{n}{2}\log 2\pi - \frac{1}{2}\log (\det \Sigma_{\bm{\theta}}) -\frac{1}{2}\bm{y}^{\top}A^{\top}\Sigma_{\bm{\theta}}^{-1}A\bm{y}
$$

方便起见，记 $A = I - X(X^{\top}\Sigma_{\bm{\theta}}^{-1}X)^{-1}X^{\top}\Sigma_{\bm{\theta}}^{-1}$ ，这里 $I$ 是 $n$ 维单位矩阵。
...
经过一些复杂的矩阵运算，$\log \mathcal{L}(\bm{\theta})$ 化简为如下形式。
