...
```{r}
# gambia$residuals <- fit_gambia_glm$residuals
...
plot(fit_gambia_village_vario, xlab = "距离（千米）", ylab = "半变差函数值")
```
...
@Diggle2002 使用贝叶斯方法来推断，采用模糊先验和马尔可夫蒙特卡洛算法获得模型参数后验估计值，并主要依赖 **geoRglm** 包实现。结果显示随着儿童年龄增长感染的可能性增加，蚊帐有保护作用但杀虫效果不显著，来自环境作用的空间效应是存在的，村庄是否被基础健康系统覆盖和周围的植被覆盖度对疟疾的流行度影响很小，至少是不显著的。模型参数的贝叶斯估计结果见下表，估计的 95\% 的置信区间端点分别来自后验 2.5\% 和97.5\% 分位点，第3列和第4列分别是后验均值和中位数。
