...
横坐标`x`、纵坐标`y`（单位：米）是村庄的位置，是否感染`pos`是响应变量，儿童年龄`age`（单位：天），是否使用蚊帐`netuse`，以及是否对蚊帐杀虫`treated`，植被覆盖度`green`，是否有医院`phc`（National Primary Health Care，简称 PHC），所属调查区域 `area`。不难理解，蚊帐可以阻挡蚊虫，对蚊帐杀虫可以增强蚊帐的隔离效果，植被越多往往蚊虫越多，有医院可以治疗疟疾。`gambia` 数据集见下 @tbl-gambia-data 。
...
数据探索主要是统计各个村庄的儿童感染疟疾的比率，以及环境因素（医院、植被）、个人因素（年龄、蚊帐）对疟疾流行度的影响，为了对数据。首先按各个村庄分组统计儿童的感染率，如下：
...
数据集 gambia 中变量 `x` 和 `y` 分别为村庄的横、纵坐标，参考系是 [UTM zone 28N](https://epsg.io/32628)，EPSG 代码是 32628。下面将数据集 gambia 从 data.frame 类型转化为 POINT 类型，并设置坐标参考系 `EPSG:32628`，接着将 `gmb_map` 数据从 SpatVector 类型转为 MULTIPOLYGON 类型，再将其参考系从 WGS 84 转为 UTM zone 28N，以便将疟疾观测数据和冈比亚行政区划数据绘制在一幅图上。
