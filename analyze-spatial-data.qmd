...
在空间线性混合效应模型一节，截距 $\beta$ ，方差 $\sigma^2$ ，块金效应 $\tau^2$ 和范围参数 $\phi$ 都估计出来了。在此基础上，采用普通克里金插值方法预测，公式见 @eq-ordinary-kriging 。
...
mean_sk <- apply(m_pred, 2, mean_fun)
# 辐射强度预测值
...
# 辐射强度预测方差
var_sk <- apply(m_pred, 2, var_fun)
rongelap_grid_df$var_sk <- var_sk
```
