...
$$
\begin{aligned}
\min_{\bm{w}} \quad & \mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} \\
...
$$
\min_{\bm{w}} \mathcal{L}(\bm{w},k) = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\bm{w}^{\top} \bm{u} + \kappa (\bm{w}^{\top} \bm{1} - 1)
$$
...
$$
\begin{aligned}
\frac{\partial \mathcal{L}(\bm{w},\kappa)}{\partial \bm{w}} &= 2 \bm{w}^{\top} V - 2 \bm{u}^{\top} + \kappa \bm{1}^{\top}= 0 \\
\frac{\partial \mathcal{L}(\bm{w},k)}{\partial \kappa} &= \bm{w}^{\top} \bm{1} - 1 = 0
\end{aligned}
$$

所以，$\bm{w}^{\top} = \bm{u}^{\top}V^{-1}$ ，在位置 $x_0$ 处的预测 $\hat{\lambda}(x_0) = \beta + \bm{u}^{\top}V^{-1}(\bm{\lambda} - \beta \bm{1})$ ，预测的均方误差 $\mathsf{E}\{[\hat{\lambda}(x_0) - \lambda(x_0)]^2\} = \sigma_0^2 - \bm{u}^{\top}V^{-1}\bm{u}$ 。

作为对比，在上一节空间线性混合效应模型的基础上，简单起见，不考虑块金效应，下面复用截距 $\beta$ 、指数型自相关函数的范围参数 $\phi$ ，采用简单克里金方法预测。
