...
cloud(counts / time ~ cX * cY,
  ...
  panel = function(...) { # 添加海岸线
    L <- list(...)
    L$x <- rongelap_coastline$cX
    L$y <- rongelap_coastline$cY
    L$z <- rep(0, nrow(rongelap_coastline))
    L$type <- "l"
    L$col <- "black"
    L$lty <- 2
    L$lwd <- 1
...
rongelap$dummary <- 1
# 拟合存在异方差的一般线性模型
fit_rongelap_lme <- nlme::lme(
  log(counts / time) ~ 1, data = rongelap, random = ~ 1 | dummary,
  correlation = nlme::corExp(
    value = c(200, 0.1), form = ~ cX + cY, nugget = TRUE
  )
)
...
::: callout-note
[**glmmTMB**](https://github.com/glmmTMB/glmmTMB) 包基于 [TMB](https://github.com/kaskr/adcomp)[@TMB2016]框架可以拟合空间广义线性混合效应模型，使用语法和前面介绍的函数 `glm()` 类似。
...
函数 `glmmTMB()` 还支持梅隆型 `mat` 和高斯型 `gau` 等协方差结构。它返回一个 glmmTMB 类型的数据对象，提供的方法如下：
...
对数似然的值和 **spaMM** 包的输出结果接近，但 **glmmTMB** 包不能估计协方差函数中的参数 $\phi$。值得注意，由于 $\sigma^2$ 与 $\tau^2$ 存在识别困难， **spaMM** 与 **glmmTMB** 都没有给出块金效应的估计。
:::
