...
Paulo J. Ribeiro Jr 将疟疾调查数据、冈比亚地区边界数据打包在 **geoR** 包内，加载和查看调查数据的代码如下：
...
横坐标`x`、纵坐标`y`（单位：米）是村庄的位置，是否感染`pos`是响应变量，儿童年龄`age`（单位：天），是否使用蚊帐`netuse`，以及是否对蚊帐杀虫`treated`，植被覆盖度`green`，是否有医院`phc`（National Primary Health Care，简称 PHC）。不难理解，蚊帐可以阻挡蚊虫，对蚊帐杀虫可以增强蚊帐的隔离效果，植被越多往往蚊虫越多，有医院可以治疗疟疾。gambia 数据集见下 @tbl-gambia-data 。
...
```{r}
fit_gambia_glm <- glm(pos ~ age + netuse + treated + green + phc,
...
fit_gambia_spamm <- fitme(pos ~ age + netuse + treated + 
                            green + phc + Matern(1 | x + y),
  family = binomial(link = "logit"), data = gambia
)
summary(fit_gambia_spamm)
```

从输出结果来看，自协方差函数中参数 $\rho = \sqrt{2 \times 0.196} / 3.0419 \times 10^5 = 20582.5$，$\nu = 0.196$。随着距离增加，相关性递减，近邻相关性辐射最远为 20.5825 千米。
