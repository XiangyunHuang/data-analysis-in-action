...
上式第一个等号右侧期望可以用样本来计算，称之为样本半变差函数，第二个等号右侧为理论半变差函数。样本半变差函数 $\hat{\gamma}(u_{ij})$ 的计算公式如下：
...
**spaMM** 包可以拟合空间广义线性混合效应模型，它的使用语法与前面介绍的函数 `glm()` 和 **nlme** 包类似，函数 `fitme()` 可以拟合从线性模型到广义线性混合效应模型的一大类模型，且使用统一的语法，输出一个 HLfit 类型的数据对象。 **spaMM** 包的函数 `Matern()` 实现了梅隆型自协方差函数，指数型和幂二次指数型是它的特例。当固定 $\nu = 0.5$ 时，梅隆型自相关函数 `Matern()` 的形式退化为 $\exp(-\rho d)$ ，其中 `d` 表示距离，相当于前面出现的 $u$，$\rho$ 相当于前面出现的 $1/\phi$ 。
