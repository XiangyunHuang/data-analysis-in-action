...
截距 $\beta$ 相当于平均水平，$\lambda(x_i)$ 可理解为辐射强度，$S(x_{i})$ 表示位置 $x_i$ 处的空间效应，$S(d),d \in \mathbb{R}^2$ 是平稳空间高斯过程。且 $Z_i$ 之间相互独立同正态分布 $\mathcal{N}(0,\tau^2)$ ，$Z_i$ 表示与空间效应无关的块金效应，即非空间的随机效应，可以理解为测量误差、空间变差或背景辐射，这里 $i = 1,\ldots, 157$。
...
接下来用经验半变差函数图检查空间相关性， rongelap 数据集包含 157 个采样点的坐标，两两配对，共有 $(157 - 1) \times (157 - 2) / 2 = 12090$ 对。拟合残差的半变差函数
...
fit_rongelap_vario
```
结果显示，距离在 0-89.44272 米之间的坐标点有 510 对，距离在 89.44272-144.22205 米之间的坐标点有 601 对，依此类推。将距离和计算的半变差绘制出来，即得到经验半变差图，如 @fig-rongelap-vario 所示。刚开始，半变差很小，之后随距离增加而增大，直到达到一个平台。由于半变差反比于空间相关性的程度，随着距离增加，空间相关性减弱。这说明数据中确含有空间相关性，模型中添加指数型空间自相关结构是合理的。
