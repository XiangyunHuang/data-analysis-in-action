...
空间分析的内容十分广泛，本章仅以一个模型和两个数据简略介绍。一个模型是空间广义线性混合效应模型，空间广义线性混合效应模型在卫生统计、气象统计等领域有广泛的应用，如预测某地区内的疟疾流行度分布，预测某地区 PM 2.5 污染物浓度分布等。两个数据分别来自生态环境和流行病学领域，数据集所含样本量都不大，但每个样本收集成本不小，采集样本前也都有实验设计，收集数据的地点是固定的。

2002 年张首次提出蒙特卡洛极大似然算法给出空间广义线性混合效应模型的估计与预测[@Zhang2002]，因混合模型的多样性，参数计算的复杂性，后续学者们分别从贝叶斯和频率方法的视角提出精确和近似的算法。近似算法的想法是找了一个近似函数替换其中的部分或者全部，使得随着迭代次数增加的情况下，估计的参数值可以趋于真值。精确算法的想法是通过模拟计算的方式硬积一个高维积分，不断增加采样的次数逼近目标函数，比如基于 MCMC 及其改进型 HMC 和 NUTS 硬积参数的联合后验分布中的高维积分做参数的全贝叶斯推断。贝叶斯方法主流的有拉普拉斯近似和全贝叶斯推断，频率方法主流的有基于似然的拉普拉斯近似和自适应高斯-埃尔米特算法。值得一提，**spaMM** 包[@Rousset2014]，**INLA** 包[@Rue2009; @Lindgren2015]、**glmmfields** 包[@glmmfields2018]和 **sdmTMB** 包[@Anderson2022]也可以用来拟合模型。
...
[Ole F. Christensen](https://orcid.org/0000-0002-8230-8062) 和 Paulo J. Ribeiro Jr 将 `rongelap` 数据集存放在 [**geoRglm**](https://cran.r-project.org/package=geoRglm)[@Christensen2002] 包内，后来，**geoRglm** 不维护，已从 CRAN 移除了，笔者从他们主页下载了。数据集 `rongelap` 记录了 157 个测量点的伽马射线强度，即在时间间隔 `time` （秒）内放射的粒子数目 `counts`（个），测量站点的横纵坐标分别为 `cX` （米）和 `cY`（米），坐标参考原点在岛的东北， @fig-rongelap-location 中右上角的位置。下 @tbl-rongelap-nuclear-data 展示部分朗格拉普岛核污染检测数据及海岸线坐标数据。
...
::: callout-note
当 `family = poisson(link = "log")` 时，响应变量只能放正整数，所以不能放 `counts / time`。泊松广义线性模型是对辐射强度建模，辐射强度与位置 `cX` 和 `cY` 有关。当响应变量为放射出来的粒子数目 `counts` 时，为了表示辐射强度，需要设置参数 `offset`，表示与放射粒子数目对应的时间间隔`time`。联系函数是对数函数，因此时间间隔需要取对数。
:::
...
进一步，去掉随机效应相互独立的假设，随机效应之间存在相关性结构，这更符合位置效应存在相互影响的实际情况，结合 @fig-rongelap-vario ， 核辐射强度的经验变差图呈现的空间相关性，也因此要引入带空间随机效应的广义线性混合效应模型。
...
模型中，截距 $\beta$ 相当于平均水平，放射粒子数$Y_{i}$作为响应变量服从均值为 $t(d_{i})\lambda(d_i)$ 的泊松分布，$\lambda(d_i)$ 可理解为辐射强度，$S(d_{i})$ 表示位置 $d_i$ 处的空间效应，随机过程 $S(d)$ 的自协方差函数常用的有指数型和 Matérn 型，形式如下：
...
[**glmmTMB**](https://github.com/glmmTMB/glmmTMB) 包 [@Brooks2017] 基于 [TMB](https://github.com/kaskr/adcomp)[@TMB2016]框架可以拟合空间广义线性混合效应模型，使用语法和前面介绍的 **lme4** 包一样。**glmmTMB** 包还可以拟合线性和广义线性混合效应模型及其扩展，包括零膨胀模型，估计方法是极大似然，实现工具是 TMB。随机效应假定是高斯分布，拉普拉斯方法近似随机效应的高维积分，自动微分的方式计算对数似然函数的梯度。
...
其中协方差结构由核函数决定，这里采用广泛使用的 Matérn 型，**glmmTMB** 包还支持指数型`exp` 和高斯型 `gau`，协方差矩阵是 $157 \times 157$ 维的。对数似然函数的值又变大了，拟合效果更好了，相比于从广义线性模型到广义线性混合效应模型，提升不是特别多，一般来说，模型越复杂提升空间会越小，过于复杂也没有必要。
...
从输出结果来看，与 **glmmTMB** 很接近，模型固定效应的截距项 1.824，随机效应的方差为 0.2973，自协方差函数中参数 $\nu = 0.3642$，对比 **spaMM** 包的函数 `Matern()` 实现的 Matérn 型自相关函数公式与 @eq-matern-formula ，输出结果需要转化一下，以符合 @eq-matern-formula ，则 $\rho = \sqrt{2 \times 0.364211759} / 0.0061486 = 138.8079$ ，即空间相关性的最大影响距离为 138.8 米，对数似然函数值为 -1317.551，拟合效果的提升几乎可以忽略。
...
为了确保覆盖整个岛，处理好边界问题，还需要一点缓冲空间，就是说在给定的边界线外围再画一个多边形，这可以用函数 `st_buffer()` 根据海岸线构造缓冲区，一个 POLYGON 类型的几何数据对象，缓冲距离选择 50 米。
...
网格的中心点将作为新的采样点，要在这些新的采样点上预测核污染浓度。全岛网格化后，将朗格拉普岛海岸线、网格都展示出来，如 @fig-rongelap-grid 所示。
...
美国当年是在比基尼环礁做的氢弹核试验，试验地与朗格拉普岛相距 100 多英里，核辐射羽流受大气、海洋环流等影响，漂流到朗格拉普岛，又受朗格拉普岛周围水文、地理环境影响，核污染在全岛的分布是不均匀的，颜色越深表示核污染越严重。

## 预测疟疾的流行度 {#sec-gambia-children-malaria}

本节任务是基于 **geoR** 包内置的 Gambia 数据集，通过分析 5 岁以下儿童在非洲冈比亚 65 个村庄的疟疾感染情况，建模预测疟疾流行度。

::: callout-note
[Paulo J. Ribeiro Jr](http://www.leg.ufpr.br/~paulojus/) 和 [Peter J. Diggle](https://www.lancaster.ac.uk/staff/diggle/) 将 Gambia 数据集内置在 **geoR** 包内，[Barry Rowlingson](https://barry.rowlingson.com/research.html) 也分析过 Gambia 数据集 [@Thomson1999; @Diggle2002]。2019 年 Paula Moraga 在书《Geospatial Health Data: Modeling and Visualization with R-INLA and Shiny》中[第八章](https://www.paulamoraga.com/book-geospatial/sec-geostatisticaldataexamplespatial.html) 借助 **INLA** 包基于 Gambia 数据集在村庄水平下分析疟疾流行度。
:::

### 数据说明 {#sec-gambia-data}

冈比亚国家发起基础健康服务（Primary Health Care，简称 PHC）项目 ，在 2-3 年内建立覆盖全国各个村庄的服务站点，向村民们发放蚊帐，而后，国家蚊帐杀虫项目对各村庄展开调查，收集蚊帐发放、使用情况，对发放的蚊帐杀虫，根据儿童血液样本中是否含有疟原虫确定是否感染疟疾，调查疟疾的流行度，调查在 1992 年雨季前完成，记录了 65 个村庄 5 岁以下儿童是否感染疟疾的情况，一共收集了 2035 个样本。冈比亚是非洲西部的一个贫穷小国家，国内冈比亚河横穿全国，流入大西洋，当地属热带气候，水网密布。下面借助 [**geodata**](https://github.com/rspatial/geodata)包获取[SRTM](https://srtm.csi.cgiar.org/)（Shuttle Radar Topography Mission，简称 SRTM）和 [GADM](https://gadm.org/)（Database of Global Administrative Areas，简称 GADM）开放的地形和行政区划数据，绘制冈比亚地图。SRTM 对纬度范围在南纬 60 至北纬 60 度的地区，提供 90 米精度的海拔数据，GADM 开放全球各国的各级行政区划数据。**geodata** 获取的海拔数据，返回 SpatRaster 类型的 **terra** 数据对象，它是一个栅格数据。**geodata** 获取的二级级行政区数据，返回 SpatVector 类型的 **terra** 数据对象，它是一个多边形边界地图数据。**geodata** 包下载数据的代码如下：
...
下 @fig-gambia-map 是用 **ggplot2** 包和**tidyterra**包绘制的，[**tidyterra**](https://github.com/dieghernan/tidyterra) 包基于 [**terra**](https://github.com/rspatial/terra) 包提供一些专门用于栅格数据可视化的图层，扩展了 **ggplot2** 包的功能。国家基础设施建设落后，医疗卫生条件比较差，越靠近河流的地方，水草越多，蚊虫也越多，疟疾容易通过蚊虫叮咬感染和传播。如 @fig-gambia-map 所示，冈比亚国家整体上海拔比较低，最高的地方不超过 70 米，沿着冈比亚河，各个村庄分布在南、北岸。

```{r}
#| label: fig-gambia-map
#| fig-cap: "冈比亚地形图"
#| fig-width: 8
#| fig-height: 2.75
#| fig-showtext: true
...
ggplot() +
  ...
  tidyterra::scale_fill_whitebox_c(palette = "viridi") +
  tidyterra::geom_spatvector(
    data = gmb_map, fill = NA, colour = "gray40"
  ) +
  theme_minimal() +
  labs(x = "经度", y = "纬度", fill = "海拔")
```
...
library(terra)
plot(gmb_alt, fun = function() lines(gmb_map), col = hcl.colors(50))
```

Paulo J. Ribeiro Jr 将疟疾调查数据、冈比亚地区边界数据打包在 **geoR** 包内，在数据集上添加了区域标记后，加载和查看调查数据的代码如下：

```{r}
# Gambia 儿童疟疾感染数据
gambia <- readRDS(file = "data/gambia.rds")
# 查看 gambia 数据集
str(gambia)
```

数据集 `gambia` 中变量 `x` 和 `y` 分别为村庄的横、纵坐标，单位：米，坐标参考系是 [UTM zone 28N](https://epsg.io/32628)，对应的 EPSG 代码是 32628。是否感染`pos`是响应变量，儿童年龄`age`（单位：天），是否使用蚊帐`netuse`，以及是否对蚊帐杀虫`treated`，植被覆盖度`green`，是否有医院`phc`（National Primary Health Care，简称 PHC），所属调查区域 `area`。不难理解，蚊帐可以阻挡蚊虫，对蚊帐杀虫可以增强蚊帐的隔离效果，植被越多往往蚊虫越多，有医院可以治疗疟疾。`gambia` 数据集见下 @tbl-gambia-data 。
...
### 数据探索 {#sec-gambia-exploration}

数据探索主要是统计各个村庄的儿童感染疟疾的比率，以及环境因素（医院、植被）、个人因素（年龄、蚊帐）对疟疾流行度的影响。首先按各个村庄分组统计儿童的感染率，如下：

```{r}
# 统计各村庄的感染情况
gambia_village <- aggregate(
  data = gambia,
  cbind(ncases = pos, ncontrols = 1 - pos) ~ x + y + green + phc,
  FUN = sum
)
```

为了查看感染率的空间分布，需要将疟疾观测数据和冈比亚行政区划数据绘制在一幅图上。下面将数据集 `gambia` 从 data.frame 类型转化为 POINT 类型，并设置坐标参考系 `EPSG:32628`，接着将 `gmb_map` 数据从 SpatVector 类型转为 MULTIPOLYGON 类型，再将其参考系从 WGS 84 转为 UTM zone 28N。

```{r}
library(sf)
gambia_village <- st_as_sf(gambia_village, coords = c("x", "y"), crs = 32628)
gambia_borders <- st_as_sf(gmb_map)
gambia_borders <- st_transform(gambia_borders, crs = 32628)
```

远离河流，蚊虫相对会少一些，海拔应该也是重要的变量，因此，根据冈比亚海拔数据获取各个村子的海拔高度，以便后续放入模型。下面根据 `gambia_village` 从 `gmb_alt` 中抽取海拔数据，并将获得的海拔数据添加到 `gmb_alt` 数据集上。在模型中，建议坐标的单位转为米，所以输入模型的观测数据的坐标参考系为 `EPSG:32628`，由于 **geodata** 包下载的海拔数据坐标参考系为 `EPSG:4326` ，所以需要转化一下再提取各个村庄的海拔。代码如下：

```{r}
# 转化为 WGS 84 坐标系
gambia_agg2 <- st_transform(gambia_village, crs = 4326)
# 提取各个村庄的海拔数据
gambia_village$alt <- terra::extract(gmb_alt, gambia_agg2, ID = FALSE)
```

如 @fig-observed-prevalence 所示，图中的点为村庄位置，感染比例越高点越亮，反之，点越暗。根据冈比亚狭长的地形和横穿东西全境的冈比亚河，可将其看作东、中和西三个部分，东部是河流上游，西部是河流下游，村庄分为 5 个区域，西部两个，南岸和北岸各一个，中游一个在南岸，东部两个，也是南岸和北岸各有一个。
...
# 调查区域标记
geom <- st_sfc(
  st_point(c(-16.2, 13.1)),
  st_point(c(-16.1, 13.65)),
  st_point(c(-15, 13.42)),
  st_point(c(-14, 13.62)),
  st_point(c(-14, 13.25))
)
sf_area <- st_sf(area = 1:5, geometry = geom, crs = 4326)
# 主要部分
p <- ggplot() +
  geom_sf(data = gambia_borders, fill = NA) +
  geom_sf(data = gambia_village, aes(color = ncases / (ncases + ncontrols))) +
  scale_color_viridis_c(labels = scales::percent) 

p1 <- p +
  theme_minimal() +
  labs(x = "经度", y = "纬度", color = "感染率")
p2 <- p + 
  geom_sf_label(data = sf_area, aes(label = area)) +
  coord_sf(xlim = c(340000, 410000)) +
  theme_void() +
  theme(legend.position = "none")
p3 <- p + 
  geom_sf_label(data = sf_area, aes(label = area)) +
  coord_sf(xlim = c(480000, 530000)) +
  theme_void() +
  theme(legend.position = "none")
p4 <- p + 
  geom_sf_label(data = sf_area, aes(label = area)) +
  coord_sf(xlim = c(570000, 630000)) +
  theme_void() +
  theme(legend.position = "none")

p1
print(p2, vp = grid::viewport(x = .25, y = .225, width = .45, height = .45))
print(p3, vp = grid::viewport(x = .55, y = .725, width = .45, height = .45))
print(p4, vp = grid::viewport(x = .75, y = .225, width = .45, height = .45))
```

gambia 数据集记录了医院、蚊帐、年龄等变量，下面统计医院和蚊帐对疟疾感染的影响。医院对疟疾感染的影响是积极的，有医院相比于没有医院的地方，感染比例从 40.99% 下降至 33.29% 。

```{r}
prop.table(xtabs( ~ phc + pos, data = gambia), margin = 1)
```

类似地，统计蚊帐对疟疾感染的影响，发现也是积极的，有蚊帐相比于没有蚊帐的地方，感染比例从 48.47% 下降至 30.55% ，其作用比医院还大。

```{r}
prop.table(xtabs( ~ netuse + pos, data = gambia), margin = 1)
```

各个调查区域的感染情况差异也很大，推测和地区发展水平有关。

```{r}
prop.table(xtabs( ~ area + pos, data = gambia), margin = 1)
```

医院、蚊帐的影响都是非常积极且显著的，符合大家的认知。

### 数据建模 {#sec-gambia-model}

数据集包含个体水平和村庄水平的变量，因此有两个模型可供选择：
...
$Y_{ij}$ 表示第 $i$ 个村庄的第 $j$ 个儿童是否感染疟疾的响应变量，对应每个人血液检验的结果，如果血液中含有疟原虫，表示感染记为 1，否则，没有感染记为 0。$p_{ij}$ 表示第 $i$ 个村庄的第 $j$ 个儿童的感染率，是伯努利分布的成功概率，实际含义是儿童血液中携带疟原虫的可能性。其中，$i = 1,2,\cdots, 65$ 。每个儿童感染可能性与很多因素相关，既有环境因素（周围植被，基础医疗），也有个体因素（年龄、使用蚊帐的情况），建立如下空间广义线性混合效应模型。
...
如果同一村庄内儿童感染疟疾不是条件独立的，则可以考虑添加随机效应 $U_i$，即认为给定 $U_i$ 和 $S(x_i)$ 的情况下，儿童感染疟疾是独立的。上式中 $\alpha$ 是截距， $\boldsymbol{\beta}^{\top}$ 是回归参数向量，各个分量的含义如下表，$S(x_i)$ 和 $U_i$ 的含义同 @eq-matern-formula 。
...
| $\beta_1$ | 儿童年龄(age)，以天计          |
| $\beta_2$ | 使用蚊帐(bednet)               |
| $\beta_3$ | 蚊帐有杀虫(treated)            |
| $\beta_4$ | 村庄周围植被覆盖度(greenness)  |
| $\beta_5$ | 村庄是否被医疗中心覆盖(phc)    |
...
$Y_{i}$ 表示第 $i$ 个村庄感染疟疾的儿童数量，是响应变量，对应整个村子的情况。$N_i$ 表示第 $i$ 个村庄的样本量，$p_{i}$ 表示第 $i$ 个村庄的感染率，其中，$i = 1,2,\cdots, 65$ 。$\boldsymbol{\beta}^{\top}$ 表示村庄的绿色植被覆盖度 `green`、是否被基础健康保障覆盖 `phc`、所属调查区域 `area` 等影响因素的回归系数。

$$
\begin{aligned}
\log\{p_{i}/(1-p_{i})\} &= \alpha + \boldsymbol{\beta}^{\top}z_{i} + S(x_i) \\
\log\{p_{i}/(1-p_{i})\} &= \alpha + \boldsymbol{\beta}^{\top}z_{i} + S(x_i) + U_{i}
\end{aligned}
$$ {#eq-gambia-village}

个体水平下的模型可以在五个调查区域内提供个体水平下（单个儿童）感染疟疾的可能性预估，村庄水平下的模型可以在冈比亚全国提供村庄水平下（整个村庄）疟疾感染率的预估，由于有限的观测变量和数据，如缺乏覆盖全国的 `green` 和 `phc` 等数据，对全国范围内的预测变量只有海拔高度 `alt`。

$$
\log\{p_{i}/(1-p_{i})\} = \alpha + \mathrm{alt}_i*z_{i} + S(x_i) + U_{i}
$$ {#eq-gambia-predict}

作为对比的基础，首先考虑不含空间效应 $S(x_i)$ 和随机效应 $U_i$ 的广义线性模型

$$
\log\{p_{ij}/(1-p_{ij})\} = \alpha + \boldsymbol{\beta}^{\top}z_{ij}
$$ {#eq-gambia-baseline}

```{r}
fit_gambia_glm <- glm(pos ~ age + netuse + treated + green + phc + area,
  data = gambia, family = binomial(link = "logit")
)
summary(fit_gambia_glm)
```

年龄越大越易感染疟疾，蚊帐和基础医疗的保护作用都是显著的，相对于区域 1，区域 2 和区域 3 的感染率贡献更低，而区域 5 对感染率的贡献明显更高。杀虫对降低感染率是有正向作用的，植被覆盖越高感染疟疾可能性会增加，但杀虫效果和植被覆盖度都没有达到统计上的显著性。模型的对数似然如下：

```{r}
logLik(fit_gambia_glm)
```

根据广义线性模型的结果，$\hat{p}_{ij}$ 是 $Y_{ij}$ 的期望，可以提取预测偏差 $Y_{ij}-\hat{p}_{ij}$ 。

```{r}
gambia$residuals <- residuals(fit_gambia_glm, type = "deviance")
```

以及根据儿童的数据，拟合出来的感染疟疾的概率 $\hat{p}_{ij}$ 。

```{r}
gambia$pred_prob <- predict(fit_gambia_glm, type = "response")
```

根据村庄每个儿童的预测感染概率，得到村庄水平下的感染概率 $p_i$ 和残差 $(Y_{i} - p_{i}) / \sigma_i$，其中 $Y_i = \sum_{j=1}^{n_i} Y_{ij}$，而 $n_i$ 表示第 $i$ 个村庄的儿童数，则 $Y_i$ 表示第 $i$ 个村庄感染的人数。$p_i = \sum_{j=1}^{n_i}\hat{p}_{ij}$ 对应于 $Y_i$ 的期望，$\sigma_i^2 = \sum_{j=1}^{n_i}\hat{p}_{ij}(1-\hat{p}_{ij})$ 对应于 $Y_i$ 的方差。

```{r}
# 统计各村庄的感染情况
gambia_village2 <- aggregate(
  data = gambia,
  cbind(
    total = 1, pos, pred_prob,
    sigmasq = pred_prob * (1 - pred_prob)
  ) ~ x + y + green + phc, FUN = sum
)
gambia_village2 <- within(gambia_village2, {
  residuals <- (pos - pred_prob) / sqrt(sigmasq)
})
```
...
# 根据村庄水平下的预测值和拟合误差
plot(residuals ~ fitted, data = gambia_village2)
# 残差的空间分布
ggplot(data = gambia_village2) +
  geom_point(aes(x = x, y = y, color = residuals)) +
  colorspace::scale_color_continuous_diverging(palette = "Blue-Red") +
  theme_minimal()
```

剔除村庄相关的随机效应（非空间的），拟合残差趋势，得到变差函数。

```{r}
fit_gambia_village <- nlme::gls(residuals ~ 1, data = gambia_village2)
fit_gambia_village_vario <- nlme::Variogram(fit_gambia_village,
  form = ~ x + y, data = gambia_village2, resType = "response"
)
```

根据经验半变差图检查残差中是否存在空间相关性。如果残差的半变差图呈现一条水平线，随着距离的增加，半变差函数值没有变化，则意味着没有空间相关性，否则，存在相关性空间结构。

```{r}
#| label: fig-gambia-village-vario
#| fig-cap: "残差的半变差图"
#| fig-showtext: true
#| fig-width: 5
#| fig-height: 4

plot(fit_gambia_village_vario,
  col.line = "black", scales = list(
    # 去掉图形上边、右边多余的刻度线
    x = list(alternating = 1, tck = c(1, 0)),
    y = list(alternating = 1, tck = c(1, 0))
...
从上图可以看出空间相关性不是很强了，之前已在模型中添加空间区域变量，解释了大部分空间因素，此时，若在模型中添加空间效应 $S(x_i)$ ，预计效果提升不会很多。下面拟合带空间效应的广义线性混合效应模型：

```{r}
library(spaMM)
fit_gambia_spamm <- fitme(pos ~ age + netuse + treated + 
                            green + phc + area + Matern(1 | x + y),
  family = binomial(link = "logit"), data = gambia
)
summary(fit_gambia_spamm)
```

从输出结果来看，对数似然从 -1214.828 提升至 -1174.951，提升不是特别大，自协方差函数中参数 $\rho = \sqrt{2 \times 0.2587} / 3.154 \times 10^4 = 2280.613$，$\nu = 0.2587$。随着距离增加，相关性递减，近邻相关性辐射最远为 2280.613 米，即约2.3公里。

::: callout-tip
设置梅隆型自协方差函数中超参数 $\nu = 0.5$， 则模型中的自协方差函数退化为指数型，在函数 `fitme()` 中设置参数 `fixed`，代码如下：
...
@Diggle2002 使用贝叶斯方法来推断，采用模糊先验和马尔可夫蒙特卡洛算法获得模型参数后验估计值，并主要依赖 **geoRglm** 包实现。结果显示随着儿童年龄增长感染的可能性增加，蚊帐有保护作用但杀虫效果不显著，来自环境作用的空间效应是存在的，村庄是否被基础健康系统覆盖和周围的植被覆盖度对疟疾的流行度影响很小，至少是不显著的。模型参数的贝叶斯估计结果见下表，下表是在不考虑 $U_i$ 的情况下得到的结果，按照本书的记号，原文中的 $\delta$ 相当于 $\nu$，则 $\nu = 0.079522$ （取值以2.5%分位点为例，下同），而 $\phi$ 相当于 $\rho / \sqrt{2\nu}$，则 $\rho = 0.915789 * \sqrt{2 * 0.079522} = 0.3652196$。估计的 95% 的置信区间端点分别来自后验 2.5% 和97.5% 分位点，第3列和第4列分别是后验均值和中位数。
...
### 模型预测 {#sec-gambia-predict}

接下来的过程与上一个示例基本相同了。在有限的数据情况下，下面基于模型 @eq-gambia-predict 给出预测。首先，看下数据情况
