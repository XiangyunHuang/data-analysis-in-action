...
接下来，预测给定的边界（海岸线）内的任意位置的核污染浓度，展示全岛的污染图景。先从点构造多边形数据，再将多边形做网格划分，继而将格点作为模型输入获得预测，预测各个格点处的辐射强度。
...
为了确保覆盖整个岛，还需要一点缓冲空间，就是说在给定的边界线外围再画一个多边形，这可以用 函数 `st_buffer()` 根据海岸线构造 POLYGON 的缓冲区，缓冲距离选择 50 米。
