...
```{r}
library(sf)
gambia_agg <- st_as_sf(gambia_agg, coords = c("x", "y"), crs = 32628)
gambia_borders <- st_as_sf(gmb_map)
gambia_borders <- st_transform(gambia_borders, crs = 32628)
```

远离河流，蚊虫相对会少一些，海拔应该也是重要的变量，因此，根据冈比亚海拔数据获取各个村子的海拔高度，以便后续放入模型。下面根据 `gambia_agg` 从 `gmb_alt` 中抽取海拔数据，并将获得的海拔数据添加到 `gmb_alt` 数据集上，代码如下：

```{r}
# 转化为 WGS 84 坐标系
gambia_agg2 <- st_transform(gambia_agg, crs = 4326)
# 提取各个村庄的海拔数据
gambia_agg$alt <- terra::extract(gmb_alt, gambia_agg2, ID = FALSE)
```
...
gambia_village <- within(gambia_village, {
...
# 根据村庄水平下的预测值和拟合误差
plot(residuals ~ fitted, data = gambia_village)
# 残差的空间分布
ggplot(data = gambia_village) +
...
```{r}
fit_gambia_village <- nlme::gls(residuals ~ 1, data = gambia_village)
fit_gambia_village_vario <- nlme::Variogram(fit_gambia_village,
  form = ~ x + y, data = gambia_village, resType = "response"
)
```
