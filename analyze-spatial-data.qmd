...
ggplot() +
  ...
  geom_sf(data = rongelap_grid_centroid, size = 0.75) +
  theme_void()
```

抽取网格中心点的坐标并存储为 data.frame 类型，以便输入模型中。
...
将数据输入 **spaMM** 包拟合的模型，将模型返回的结果整理成数据框，并与采样点数据合并。
...
ggplot() +
  geom_sf(data = rongelap_grid_sf, aes(color = pred / time), show.legend = FALSE) +
  geom_sf(
...
为了绘制热图，利用 **stars** 包将数据框整理、转化成栅格对象。

```{r}
library(abind)
library(stars)
rongelap_grid_stars <- st_rasterize(rongelap_grid_sf, nx = 50, ny = 25)
```

使用 **ggplot2** 包绘制放射强度的热力分布图。
