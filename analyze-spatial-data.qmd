...
$$
\hat{\beta}_{gls} = (X^{\top}\Sigma^{-1}X)^{-1} X^{\top}\Sigma^{-1}\bm{y}
$$
...
$$
2 \underbrace{X^{\top}\bm{w}}_{\bm{x_0}^{\top}} -2 X^{\top}\Sigma_{11}^{-1}\Sigma_{10} -  X^{\top}\Sigma_{11}^{-1}X\bm{\kappa}= 0
$$
...
$$
\bm{w} = \Sigma_{11}^{-1} (\Sigma_{10} 
+ X(X^{\top}\Sigma_{11}^{-1}X)^{-1}(\bm{x}_0 - X^{\top}\Sigma_{11}^{-1}\Sigma_{10})) .
$$
...
&= \Sigma_{01} \Sigma_{11}^{-1}\bm{y} + (\bm{x}_0 - \Sigma_{01}\Sigma_{11}^{-1}X) (X^{\top}\Sigma_{11}^{-1}X)^{-1} X^{\top}\Sigma_{11}^{-1}\bm{y} \\
&= \bm{x}_0 \hat{\beta}_{gls} + \Sigma_{01} \Sigma_{11}^{-1}(\bm{y} - X\hat{\beta}_{gls})
\end{aligned}
$$
...
其中，$\kappa$ 表示拉格朗日乘子，$\bm{1}$ 表示 $N \times 1$ 维的全 1 向量。分别对 $\bm{w}$ 和 $\kappa$ 求偏导，并令其等于 0 ，则有
