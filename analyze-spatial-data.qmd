...
现在的目标是找到一个 $\hat{\bm{\beta}}$ ，使得残差平方和最小，即求解如下无约束最优化问题。
...
将 $\hat{\bm{\epsilon}}= \bm{y} - X\hat{\bm{\beta}}$ 代入并展开，目标函数可进一步表达为如下形式。
...
$$
-2X^{\top}\bm{y} + 2X^{\top}X\hat{\bm{\beta}} = 0
$$
...
$$
\begin{aligned}
\mathsf{Var}\{\hat{\bm{\beta}}\} &= \mathsf{E}\{\hat{\bm{\beta}}^{\top}\hat{\bm{\beta}}\} - \bm{\beta}^{\top}\bm{\beta} \\
