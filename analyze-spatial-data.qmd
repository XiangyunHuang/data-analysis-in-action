...
接下来，利用函数 `st_make_grid()` 根据海岸缓冲线的边界打网格，朗格拉普到是狭长的，因此，网格是 $45\times 75 $ 的，意味着水平方向 45 行，垂直方向 75 列，细密程度是可以调整的，越密岛内的格点越多，核污染浓度计算越精确，同时也越耗时。

```{r}
rongelap_coastline_grid <- st_make_grid(rongelap_coastline_buffer, n = c(75, 45))
```
...
**sf** 包的函数 `st_within()` 可以将小网格落在缓冲区以内的筛选出来，再用函数 `st_centroid()` 计算余下小网格的中心点。

```{r}
rongelap_coastline_grid <- st_as_sf(rongelap_coastline_grid)
rongelap_coastline_buffer <- st_as_sf(rongelap_coastline_buffer)
rongelap_grid <- rongelap_coastline_grid[rongelap_coastline_buffer, op = st_within]
...
ggplot() +
  ...
  geom_sf(data = rongelap_grid_centroid, size = 0.5) +
  theme_void()
```
