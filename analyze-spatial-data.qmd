...
根据数据可以估计经验半变差函数 @eq-semi-variogram 的参数，从而 $V$ 、 $\sigma_{0}^2$ 和 $\bm{u}$ 都是已知的。以 $V + \tau^2I$ 替换 $V$ ，$\bm{\lambda} - \beta \bm{1}$ 替换 $\bm{\lambda}$ ，可得普通克里金插值的预测值及其均方误差。

$$
\begin{aligned}
\hat{\lambda}(x_0) &= \beta + (\bm{u}^{\top} + \frac{1- \bm{1}^{\top}(V + \tau^2I)^{-1}\bm{u}}{\bm{1}^{\top}(V + \tau^2I)^{-1}\bm{1}} \bm{1}^{\top}) (V + \tau^2I)^{-1}(\bm{\lambda} - \beta \bm{1}) \\
\mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} &= \bm{u}^{\top}(V + \tau^2I)^{-1}\bm{u} - \frac{\big(1- \bm{1}^{\top}(V + \tau^2I)^{-1}\bm{u}\big)^2}{\bm{1}^{\top}(V + \tau^2I)^{-1}\bm{1}}.
\end{aligned}
$$
