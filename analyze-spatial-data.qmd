...
其中，$[h_{i,0},h_{i,1}]$ 表示第 $i$ 个距离区间，$N(h_i)$ 表示第 $i$ 个距离区间内所有样本点对的数目，只要两个点之间的距离在这个区间内，就算是一对。`rongelap` 数据集包含 157 个采样点，两两配对，共有 $(157 - 1) \times (157 - 2) / 2 = 12090$ 对。**nlme** 包的函数 `Variogram()` 拟合模型残差的经验半变差函数：
...
在空间线性混合效应模型一节，截距 $\beta$ ，方差 $\sigma^2$ ，块金效应 $\tau^2$ 和范围参数 $\phi$ 都估计出来了。在此基础上，采用普通克里金插值方法预测。
...
其中，$\bm{y}$ 是 $N \times 1$ 的列向量，表示响应变量对应的观测数据。 $X$ 是 $N \times p$ 的矩阵，表示 $p$ 维协变量对应的 $N$ 次观测数据。$\bm{\beta}$ 是 $p \times 1$ 的列向量，表示待估的模型参数。 $\bm{\epsilon}$ 是 $N \times 1$ 的列向量，表示残差。假定 $X$ 是固定的，不是随机的，且与 $\bm{\epsilon}$ 独立。随机向量 $\bm{\epsilon}$ 的期望为 $\mathsf{E}\{\bm{\epsilon}\} = \bm{0}$，方差为 $\Sigma = \mathsf{Cov}\{\bm{\epsilon},\bm{\epsilon}\} = \mathsf{Var}\{\bm{\epsilon}\}$ 。则有

$$
\mathsf{E}\{\bm{y}|\bm{\epsilon}\} = X\bm{\beta}, \quad \mathsf{Var}\{\bm{y}|\bm{\epsilon}\} = \Sigma.
$$

现在的目标是找到一个 $\hat{\bm{\beta}}$ ，在满足无偏性的条件下，使得残差平方和最小，即求解如下最优化问题。

$$
\begin{aligned}
\min_{\bm{\beta}}\quad & \hat{\bm{\epsilon}}^{\top}\hat{\bm{\epsilon}} \\
\text{s.t.}  \quad & \mathsf{E}\{\hat{\bm{\beta}} - \bm{\beta}\} = 0
\end{aligned}
$$ {#eq-gls-beta}

根据随机向量的二次型，目标函数可进一步表达为如下形式。

$$
\begin{aligned}
\hat{\bm{\epsilon}}^{\top}\hat{\bm{\epsilon}} &= (\bm{y} - X\hat{\bm{\beta}})^{\top}(\bm{y} - X\hat{\bm{\beta}}) \\
&=\bm{y}^{\top}\bm{y} - \bm{y}^{\top}X\hat{\bm{\beta}} - (X\hat{\bm{\beta}})^{\top}\bm{y} + (X\hat{\bm{\beta}})^{\top}X\hat{\bm{\beta}} \\
&=\bm{y}^{\top}\bm{y} - 2 \bm{y}^{\top}X\hat{\bm{\beta}} + \hat{\bm{\beta}}^{\top}X^{\top}X\hat{\bm{\beta}}
\end{aligned}
$$

对 $\hat{\bm{\beta}}$ 求偏导，并令偏导等于 $\bm{0}$。

$$
-2X^{\top}\bm{y} + 2X^{\top}X\hat{\beta} = 0
$$ 可得
...
$$
\mathsf{Var}\{\hat{\bm{\epsilon}}\} = \mathsf{E}\{\hat{\bm{\epsilon}}^{\top}\hat{\bm{\epsilon}}\}
$$
...
类似地，对于一次新的观测 $\bm{x}_0$，它是一个 $p \times 1$ 的列向量，现在的目标是找到一个 $\hat{y}_0$ ，在满足无偏性的条件下，使得预测的均方误差 $\mathsf{E}\{(\hat{y}_0 - y_0)^2\}$ 最小，也就是预测方差 $\mathsf{Var}\{\hat{y}_0 - y_0\}$ 最小。不妨设 $\hat{y}_0$ 是关于 $\bm{y}$ 的线性组合，即 $\hat{y}_0 = \bm{w}^{\top}\bm{y}$ ，此处，$\bm{w}$ 是 $N \times 1$ 维的权重向量。找 $\hat{y}_0$ 的问题转为求解如下最优化问题。
...
在空间分析中，克里金插值（Kriging Interpolation）是非常基础且重要的内容，具有很好的统计性质，是最佳线性无偏预测（Best Linear Unbiased Predictor）。下面介绍插值的原理， @fig-kriging 是简单克里金插值的示意图。
...
&= \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\sum_{i=1}^{N} \mathsf{Cov}\{\lambda(x_i),\lambda(x_0)\}w_i \\
& = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2[\mathsf{Cov}\{\bm{\lambda},\lambda(x_0)\}]^{\top}\bm{w}  \\
& = \bm{w}^{\top}V\bm{w} + \sigma_{0}^2 - 2\bm{u}^{\top}\bm{w} .
\end{aligned}
$$

其中，$\sigma_{0}^2 = \mathsf{Var}\{\lambda(x_0)\} = \mathsf{Var}\{\lambda(x_i)\}= \sigma^2$ 是未知常数，$\bm{u} = \mathsf{Cov}\{\bm{\lambda},\lambda(x_0)\}$ 是未知的 $N \times 1$ 向量。采用拉格朗日乘数法将约束优化问题 @eq-kriging-optim 转为如下无约束优化问题。

$$
\min_{\bm{w},\kappa} \mathcal{L}(\bm{w},k) = \frac{1}{2}\bm{w}^{\top}V\bm{w} + \frac{1}{2}\sigma_{0}^2 - \bm{u}^{\top}\bm{w}  + \kappa (1 - \bm{1}^{\top} \bm{w})
$$
...
$$
\begin{aligned}
\mathsf{MSE} &= \mathsf{E}\{[\hat{\lambda}(x_0) - \lambda(x_0)]^2\} \\
&= \mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\}
\end{aligned}
$$

根据数据可以估计经验半变差函数 @eq-semi-variogram 的参数，从而 $V$ 、 $\sigma_{0}^2$ 和 $\bm{u}$ 都是已知的。以 $V + \tau^2\mathsf{I}$ 替换 $V$ ，$\bm{\lambda} - \beta \bm{1}$ 替换 $\bm{\lambda}$ ，可得普通克里金插值的预测值及其均方误差。

$$
\begin{aligned}
\hat{\lambda}(x_0) &= \beta + (\bm{u}^{\top} + \frac{1- \bm{1}^{\top}(V + \tau^2\mathsf{I})^{-1}\bm{u}}{\bm{1}^{\top}(V + \tau^2\mathsf{I})^{-1}\bm{1}} \bm{1}^{\top}) (V + \tau^2\mathsf{I})^{-1}(\bm{\lambda} - \beta \bm{1}) \\
\mathsf{Var}\{\hat{\lambda}(x_0) - \lambda(x_0)\} &= \sigma^2 - \bm{u}^{\top}(V + \tau^2\mathsf{I})^{-1}\bm{u}.
\end{aligned}
$$
