...
::: {.callout-note}
当 `family = poisson(link = "log")` 时，响应变量只能放正整数。辐射强度与位置有关，影响放射粒子数目的除了辐射强度还有时间。参数 `offset` 是时间间隔，放射性粒子数量 `counts` 和时间间隔有关。
:::
...
除了矢量栅格化函数 `st_rasterize()` 和栅格剪裁函数 `st_crop()` ，**stars** 包还提供栅格数据图层 `geom_stars()`，这可以和 **ggplot2** 内置的图层搭配使用，下 @fig-rongelap-pred-stars 是 **ggplot2** 包绘制的辐射强度的热力分布图。
