...
其中，$K_{\nu}$ 表示阶数为 $\nu$ 的修正的第二类贝塞尔函数，$\Gamma(\cdot)$ 表示伽马函数。如 @fig-matern-fun 所示，不难看出，$\nu$ 影响协方差函数的平滑性，控制点与点之间相关性的变化，$\nu$ 越大，意味着相关性越迅速地递减。$\rho$ 控制协方差函数的范围，$\rho$ 越大，相关性辐射距离越远。
...
下 @fig-gambia-map 是用 **ggplot2** 包和**tidyterra**包绘制的，[**tidyterra**](https://github.com/dieghernan/tidyterra) 包基于 [**terra**](https://github.com/rspatial/terra) 包提供一些专门用于栅格数据可视化的图层，扩展了 **ggplot2** 包的功能。很多非洲国家基础设施建设落后，医疗卫生条件比较差，越靠近河流的地方，水草越多，蚊虫也越多，疟疾容易通过蚊虫叮咬感染和传播。如 @fig-gambia-map 所示，冈比亚国家整体上海拔比较低，最高的地方不超过 100 米，沿着冈比亚河，各个村庄分布在南、北岸。
...
横坐标`x`、纵坐标`y`（单位：米）是村庄的位置，是否感染`pos`是响应变量，儿童年龄`age`（单位：天），是否使用蚊帐`netuse`，以及是否对蚊帐杀虫`treated`，植被覆盖度`green`，是否有医院`phc`。不难理解，蚊帐可以阻挡蚊虫，对蚊帐杀虫可以增强蚊帐的隔离效果，植被越多往往蚊虫越多，有医院可以治疗疟疾。gambia 数据集记录了冈比亚地区 65 个村庄 5 岁以下儿童是否感染疟疾的情况，一共收集了 2035 个样本，见下 @tbl-gambia-data 。
