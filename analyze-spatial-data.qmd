...
从输出结果来看，与 **glmmTMB** 很接近，模型固定效应的截距项 1.824，随机效应的方差为 0.2973，自协方差函数中参数 $\nu = 0.3642$，$\rho = \sqrt{2 \times 0.364211759} / 0.0061486 = 138.8079$（对模型来说，它们是超参数），对数似然函数值为 -1317.551，拟合效果的提升几乎可以忽略。
...
从输出结果来看，自协方差函数中参数 $\nu = 0.196$，$\rho = \sqrt{2 \times 0.196} / 3.0419 \times 10^5 = 20582.5$。随着距离增加，相关性递减，近邻相关性辐射最远为 20.5825 千米。

::: {.callout-tip}
设置梅隆型自协方差函数中超参数 $\nu = 0.5$， 则模型中的自协方差函数退化为指数型，代码如下：
