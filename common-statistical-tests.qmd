...
费舍尔精确检验 `fisher.test` 固定边际的情况下，检验列联表行和列之间的独立性
...
计数数据，通俗来说，对象是一个一个或一份一份的、可数的、离散的数据，比如人数。`fisher.test()` 针对计数数据，检验列联表中行和列的独立性
...
用于计数数据的 McNemar's 卡方检验 McNemar's $\chi^2$ 检验：检验二维列联表行和列的对称性，怎么理解对称性？看帮助实例。
