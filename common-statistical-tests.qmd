...
flowchart LR
  ...
  C3 --> E6(Kruskal-Wallis 秩和检验)
  B2 --> C4(方差检验) --> E7(Bartlett 检验)
  C4 --> E8(Ansari-Bradley 检验\nMood 检验\nFligner-Killeen 检验)
```
...
flowchart LR
  ...
  B2 --> C4(方差检验) --> E6(Bartlett 检验)
  C4 --> E7(Fligner-Killeen 检验)
```
...
PlantGrowth 属于一元方差分析，观测变量只有植物干重一个变量。如果推广到多个变量，就是多元方差分析 multivariate analysis of variance
...
**dunn.test** Dunn's Test 秩和检验做多重比较
