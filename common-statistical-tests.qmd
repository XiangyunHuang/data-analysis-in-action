...
皮尔逊卡方检验（ Pearson's $\chi^2$ 检验） `chisq.test()` 常用于列联表独立性检验和方差分析模型的拟合优度检验。
...
对于单元格总样本量小于 40 或 T 小于 1 时，需采用费希尔精确检验。

#### 边际独立性

费希尔精确检验（ Fisher 's Exact 检验）：固定边际的情况下，检验列联表行和列之间的独立性 `fisher.test()` 。
...
用于分层分类数据的 Cochran-Mantel-Haenszel 卡方检验：两个枚举（分类）变量的条件独立性，假定不存在三因素的交互作用。Cochran-Mantel-Haenszel 检验 `mantelhaen.test()`

```{r}
mantelhaen.test(UCBAdmissions)
```

没有证据表明院系与性别之间存在关联。给定院系的情况下，是否录取和性别没有显著关系。
