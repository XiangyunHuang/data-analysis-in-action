...
**ggTimeSeries** [@ggTimeSeries2022] (<https://github.com/thecomeonman/ggTimeSeries>)
就好像爬楼梯似的，瀑布上的水，每年的提交量

25 年代码提交，数万提交量怎么累计起来的

<!-- 
https://bjt.name/2019/10/02/china-gdp.html 
-->

趋势和增减量都包含了。局部整体、是折线图的高级版本。
...
rversion <- readRDS(file = "data/rversion.rds")
```
...
rversion_tl <- within(rversion, {
  ...
  # 版本号为 x.1.0 为主要版本 major
  # 版本号为 x.0.1 为次要版本 minor
  status <- ifelse(grepl(pattern = "*\\.0\\.0", x = version), "big", version)
  status <- ifelse(grepl(pattern = "*\\.[1-9]\\.[0]", x = version), "major", status)
...
@fig-rversion-timeline 时间线图非常适合回顾过去、展望未来，讲数据故事
...
ggplot() +
  geom_segment(data = sub_rversion_df, aes(x = date, y = 0, xend = date, yend = position)) +
  geom_label(
    data = sub_rversion_df,
    aes(x = date, y = position, label = version, color = status), show.legend = FALSE
  ) +
  geom_point(data = sub_rversion_df, aes(x = date, y = 0, color = status), size = 3, show.legend = FALSE) +
