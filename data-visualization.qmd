...
## ggplot2 概览 {#sec-overview-of-ggplot2}
...
ggplot() +
  ...
  labs(x = "经度", y = "纬度", fill = "活跃度") +
  theme_minimal()
```

### 美国男性死亡率曲线变化 {#sec-usa-mortality}
...
在 R 环境中，加载 gapminder 数据集后，可以用 `str()` 函数查看数据集 gapminder 的情况。
...
其中，country（国家或地区）和 region （区域）是字符型变量，year （年份）、 pop （人口总数）、lifeExp （出生时的预期寿命，单位：岁）和 gdpPercap （人均 GDP，单位：美元）是数值型变量。
...
## 经验总结 {#sec-principles-of-ggplot2}
