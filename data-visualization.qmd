...
数据可视化的重要性还在于数据建模和模型评估，下 @fig-datasaurus-dozen 展示 13 个数据集在相关系数、均值、标准差等描述性统计量方面十分接近，但是数据分布千差万别。单个统计量就好比管窥蠡测，稍有不慎，我们就成了盲人摸象，可见数据可视化在洞察数据真实分布方面的重要性。
...
[**datasauRus**](https://github.com/jumpingrivers/datasauRus) 包 [@datasauRus2022] 13 个数据集，统计量非常接近，然而数据形态千差万别

回归方程式

$$y = \beta_0 + \beta_1 x + \epsilon$$
$\bar{x},\sigma_x,\bar{y},\sigma_y,\beta_0,\beta_1$
...
knitr::kable(datasaurus_summ, col.names = c(
  "数据集", "$\\bar{x}$", "$\\sigma_x$", "$\\bar{y}$", "$\\sigma_y$",
  "$\\beta_0$", "$\\beta_1$", "$R^2$" #, "调整的 $R^2$", "残差标准差"
), digits = 3, escape = FALSE)
```

其中 $\bar{x},\sigma_x$ 分别代表预测变量 $X$ 的均值和标准差，$\bar{y},\sigma_y$ 代表响应变量 $Y$ 的均值和标准差，$\beta_0,\beta_1$ 代表回归方程的截距和斜率，$R^2$ 代表模型拟合数据的程度。
...
下面将震级数据分割成三个区间，并以明暗不同的颜色表示震级大小，接着，根据斐济及周边地区的位置，选择相应的坐标参考系，最后，用 ggplot2 深度定制出 @fig-quakes-sf ，相比于 @fig-quakes 更加清晰、准确地反映了数据情况，目标区域位于南半球，横跨 180 度经线。
...
[弗朗西斯·高尔顿](https://galton.org/)（Francis Galton, 1822---1911） 是历史上著名的优生学家、遗传学家和统计学家等，是统计学中相关和回归概念的提出者，是生物遗传中回归现象的发现者。1885年，高尔顿以保密和给予金钱报酬的方式，向社会征集了 205 对夫妇及其 928 个成年子女的身高数据。目前，Randall Pruim 从原始文献中整理后，将该数据集命名为 Galton，放在 R 包 **mosaicData** [@mosaicData2022] 内，方便大家使用。篇幅所限，下 @tbl-galton 展示该数据集的部分内容。
...
Galton <- as.data.table(Galton)

knitr::kable(head(Galton), col.names = c(
  "家庭编号", "父亲身高", "母亲身高",
  "孩子性别", "孩子身高", "子女数量"
))
```

表中孩子性别一栏，M 表示男性（Male），F 表示女性（Female）。表中 1 号家庭父亲身高 78.5 英寸，母亲身高 67.0 英寸，育有 4 个成年子女，1 男 3 女，子女身高依次是 73.2 英寸、 69.2 英寸、 69.0 英寸 和 69.0 英寸。1 英寸相当于 2.54 厘米，78.5 英寸相当于 199.39 厘米，约等于 2 米的身高。
...
Galton[, .(median = mean((father + 1.08 * mother) / 2), height = mean(height)), by = .(family, sex)] |>
  ggplot(aes(x = median, y = height, fill = sex)) +
  geom_smooth(aes(color = sex),
    method = "lm", formula = "y~x",
    se = FALSE #, show.legend = FALSE
  ) +
  scale_color_brewer(palette = "Set1", labels = c(M = "男", F = "女")) +
  geom_point(pch = 21, color = "white", size = 3, alpha = 0.75) +
  stat_ellipse(type = "norm", linetype = 2) +
  scale_fill_brewer(palette = "Set1", labels = c(M = "男", F = "女")) +
  labs(x = "父母平均身高", y = "子女平均身高", fill = "性别", color = "性别") +
...
与回归紧密相连的是另一个统计概念：相关。在经济、社会领域中，很多数据指标存在相关性，接下来的这个例子基于 1977 年美国人口调查局发布的统计数据，篇幅所限，下 @tbl-state-x77 展示美国各州的部分统计数据。
...
如 @fig-coverage 所示，上下两个子图分别代表 Wald 和 Agresti-Coull 区间估计，纵坐标是覆盖概率，横坐标是参数 $p$ 的真值，图中黑虚线表示置信水平 $1-\alpha=0.95$，黑灰点线分别表示样本量 $n=10$ 和 $n=100$ 的模拟情况，不难看出，Wald 区间估计方法在小样本情况下表现很差，覆盖概率很少能达到置信水平的，而 Agresti-Coull 区间估计在 Wald 基础上添加了修正后，情况得到显著改善，详细的介绍见文献 @Blyth1960;@Lawrence2001;@Geyer2005 。此外，还有 Wilson 区间估计，也是用的更加广泛的，Base R 内置的二项比例检验函数 `prop.test()` 就是用该方法获得比例 $p$ 的区间估计[@Wilson1927]。

```{r}
#| label: fig-coverage
#| fig-cap: "二项分布参数的区间估计：覆盖概率随成功概率的变化"
#| out-width: "75%"
#| fig-width: 6
#| fig-height: 6
#| fig-showtext: true
...
ggplot(data = sim_dat, aes(x = p, y = prob, color = nsample)) +
  geom_hline(yintercept = 0.95, linetype = 2, linewidth = 1, color = "gray60") +
  geom_point() +
  geom_path() +
  annotate(geom = "text", x = 0, y = 0.95, label = "0.95",
