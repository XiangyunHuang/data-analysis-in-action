...
## ggplot2 介绍 {#sec-intro-to-ggplot2}
...
### ggplot2 概览 {#sec-ggplot2-overview}
...
### ggplot2 v.s. Base R {#sec-ggplot2-vs-base-r}
...
### Beyond ggplot2 {#sec-beyond-ggplot2}
...
ggplot(data = gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
  scale_x_log10(labels = label_log(), limits = c(100, 100000)) +
  labs(x = "人均 GDP", y = "预期寿命")
```
...
ggplot(data = gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
  scale_x_log10(labels = label_dollar(), limits = c(100, 100000)) +
  labs(x = "人均 GDP", y = "预期寿命")
```
...
ggplot(data = gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
  scale_x_log10(
    labels = label_dollar(), minor_breaks = mb, limits = c(100, 100000)
  ) +
  labs(x = "人均 GDP", y = "预期寿命")
```
...
从 1952-2007 年，gapminder 数据集是每隔 5 年从源数据中摘录一次整理的，因此，实际只有 12 年的数据。 @sec-font 根据 2007 年的数据绘制了 @fig-font ，每年的数据绘制一幅图像，12 年总共可获得 12 帧图像，再以每秒播放 2 帧图像的速度将 12 帧图像合成 GIF 动画。因此，设置这个动画总共 12 帧，每秒播放的图像数为 2。
...
ggplot(data = gapminder, aes(x = gdpPercap, y = lifeExp)) +
  ...
  scale_color_manual(values = country_colors) +
  scale_size(range = c(2, 12), labels = label_number(scale_cut = cut_short_scale())) +
  scale_x_log10(labels = label_log(), minor_breaks = mb, limits = c(100, 100000)) +
...
ggplot(data = gapminder, aes(x = gdpPercap, y = lifeExp)) +
  ...
  scale_color_manual(values = country_colors) +
  scale_size(
    range = c(2, 12), labels = label_number(scale_cut = cut_short_scale())
  ) +
  scale_x_log10(
    labels = label_log(), minor_breaks = mb, limits = c(100, 100000)
  ) +
  facet_wrap(facets = ~continent) +
  theme_classic(base_family = "Noto Sans") +
  theme(
...
一共是四个字段，分别是代码提交时记录的版本号 revision，提交代码的人 author，提交代码的时间 date 和提交代码时伴随的说明 msg。接下来，带着问题一起探索开源统计软件 R 过去 25 年波澜壮阔的历史！
...
数据集 `svn_trunk_log` 没有年份字段，但时间字段 date 隐含了年份信息，因此，新生成一个字段 year 将年份信息从 date 提取出来。
...
**ggTimeSeries** (<https://github.com/thecomeonman/ggTimeSeries>)

<!-- 
https://bjt.name/2019/10/02/china-gdp.html 
-->
...
时间线图，何时提交量达到 10000 20000 的，R 语言版本发布时间及提交量
...
更加直观地展示出节假日、休息工作日、寒暑假，比如描述学生学习规律
...
quake6 <- within(quake6, {
  date <- paste(year, month, day, sep = "-")
})

quake6$week <- data.table::week(quake6$date)
quake6$weekdays <- factor(data.table::wday(quake6$date), labels = week.abb)
...
R 语言社区提供多个 R 包绘制水平线图，

- **latticeExtra** 包出现的比较早，是 lattice 包
- **ggalt** (<https://github.com/hrbrmstr/ggalt>) 包
- **ggHoriPlot** (<https://github.com/rivasiker/ggHoriPlot>) 包 [@ggHoriPlot2021]，它依赖少，处于活跃维护中，属于 ggplot2 衍生包，笔者比较推荐。
...
[Sizing the Horizon: The Effects of Chart Size and Layering on the Graphical Perception of Time Series Visu](http://vis.berkeley.edu/papers/horizon/2009-TimeSeries-CHI.pdf)

[股价波动示例](https://timelyportfolio.blogspot.com/2012/06/horizon-plot-already-available.html)
...
<!-- 
等高线也是描述地形地势，山洼、山谷还是高山、高原，抑或是断崖、峡谷、平原。 
-->


<!-- 
terra [来自谷歌地球的卫星图像](https://humus.rocks/post/2022/06/23/using-the-terra-r-package-to-view-download-and-analyze-google-earth-engine-images/) 
-->
