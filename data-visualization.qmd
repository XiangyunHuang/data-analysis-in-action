...
<!-- 
看 10 篇文章总结对数据可视化的理解和应用

探索 Explore 解释 Explain
https://www.visualisingdata.com/2022/07/explore-explain-s3-e7-mick-yang-nur-munirah-mansoor/

目前市面上已经出版一些专著，它们有不同的特点，概览介绍，本文的特点

为什么数据可视化是重要的？数据可视化过程中，什么是重要的？目标读者是哪些？《R 语言实战》分三章介绍图形，分入门、基础和中级三层，本章亦有此意。
-->
...
Francis Galton（弗朗西斯·高尔顿）(<https://galton.org/>)
...
Galton[, .(median = mean((father + mother) / 2), height = mean(height)), by = .(family, sex)] |>
...
R 软件内置数据集 state.x77 源于 1977 年美国人口调查局发布的数据
...
ggplot(data = sim_dat, aes(x = p, y = prob, color = nsample)) +
  ...
  annotate(geom = "text", x = 0, y = 0.95, label = "0.95",
           fontface = "bold", hjust = 2.1, size = 3.5) +
  # scale_color_grey() +
