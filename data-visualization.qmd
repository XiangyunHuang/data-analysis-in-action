...
gapminder$region <- factor(gapminder$region,
  levels = c("北美", "欧洲与中亚地区", "东亚与太平洋地区", "中东与北非地区", "拉丁美洲与加勒比海地区", "南亚", "撒哈拉以南非洲地区")
)
...
其中，country（国家或地区）和 region （区域）是字符型变量，year （年份）、 pop （人口总数）、lifeExp （预期寿命，单位：岁）和 gdpPercap （人均 GDP，单位：美元）是数值型变量。
