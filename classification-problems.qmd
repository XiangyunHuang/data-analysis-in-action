...
以 iris 数据集为例，简单，方便模型和算法介绍，定位入门。分类间隔最大化，也是一个优化问题，找一条分界线，一个分割面，一个超平面划分不同的种类。本章篇幅：每个算法 4 页，共计 40 页。10 个算法的介绍按照分类思路，模型，代码和参数说明，分类性能评估。应用案例是手写数字识别。要点不是数据如何复杂，而是怎样把理论写得通俗、准确，看了之后能够应用到复杂的真实数据分析场景中去。理论解释、绘图说明、经验总结。
...
# 每组随机抽取 35 个样本 35 = 50*.7
iris_train <- iris_dt[, .SD[sample(x = .N, size = 35, replace = FALSE)], by = Species]
```
...
# 输入数据的类型必须是矩阵
iris_xgb <- xgboost(
  data = as.matrix(iris_train[, -"Species"]), 
  label = as.matrix(iris_train[, "Species"]),
...
```{r}
# ?predict.xgb.Booster
iris_pred <- predict(object = iris_xgb, newdata = iris_train_mat)
```
