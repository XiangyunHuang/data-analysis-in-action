...
library(sf)
# 美国州级多边形边界地图
us_states_map <- readRDS(file = "data/us-state-map.rds")
# 将观测数据与地图数据合并
...
library(biscale)
# 将数据根据分位点分箱
us_bi_data <- bi_class(us_states_df,
  x = Income, y = `Life Exp`,
  style = "quantile", dim = 3
)
# 创建主体地图
us_bi_map <- ggplot() +
  geom_sf(data = us_bi_data, color = "gray", fill = I("white")) +
  geom_sf(
    data = us_bi_data, aes(fill = bi_class),
    color = "gray80", linewidth = 0.05, show.legend = FALSE
  ) +
...
# 创建图例数据
us_bi_leg_data <- data.frame(expand.grid(x = 3:1, y = 3:1),
...
us_bi_map + inset_element(us_bi_leg,
  left = 0.85, bottom = 0.1, right = 1, top = 0.35
)
```
...
library(maps)
# 准备地图数据
us_state_map <- readRDS(file = "data/us-state-map.rds")
us_county_map <- readRDS(file = "data/us-county-map.rds")
# GEOID 是地理区域唯一标识
