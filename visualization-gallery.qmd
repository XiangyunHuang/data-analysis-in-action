...
以 R 软件内置的数据集 state.x77 为例展示 1975 年美国**各个州**每平方英里的人口数，即以专题地图形式展示各州人口密度，此处专题的含义是「人口」，显而易见，人口密度只是一方面，还有人口结构，结构又可以分性别、年龄、种族等等。

state.x77 数据集的 Population 列代表州人口数，数据来自 1977 年美国人口调查局发布的统计数据，单位是 1000 人，统计的是 1975 年的人口数据，原始数据可以从官网发布的年度报告[Statistical Abstract of the United States: 1977](https://www.census.gov/library/publications/1977/compendia/statab/98ed.html)获取。
...
ggplot() +
  ...
  geom_sf_label(data = us_states_df, aes(label = STUSPS)) +
  labs(
...
library(cartogram)
# 构造变形地图
us_states_carto <- cartogram_cont(us_states_df, "den", itermax = 5)
# 绘制变形地图
