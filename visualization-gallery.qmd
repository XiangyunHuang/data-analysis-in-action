...
从 Ross Ihaka 在 1997-09-18 提交第 1 次代码改动开始，下载所有的提交日志。截至 2022-12-31，代码最新版本号为 83528，意味着代码仓库已存在 8 万多次提交。
...
us_bi_leg <- ggplot(
  ...
  scale_fill_identity() +
  labs(
    x = substitute(paste("预期寿命", "" %->% "")),
    y = substitute(paste("人均 GDP", "" %->% ""))
  ) +
  coord_fixed() +
  theme_void() +
  theme(
...
静态的三维图形限制用户只能从一个视角观察，因视角和透视的关系，存在遮挡和视觉错位的情况，比如火山地形 @fig-volcano-wireframe 因坡面遮挡了火山口及其周围地势，只有调整观察角度，重新绘图方可，如 @fig-volcano-viewport 所示，火山口及其周围地势一目了然。
...
x <- 1:nrow(volcano)
y <- 1:ncol(volcano)
persp3d(x = x, y = y, z = volcano, col = "lightblue",
        xlab = "南北方向", ylab = "东西方向", zlab = "高度")

# 透视图是如何制作的？栅格点是如何变成曲面的？差值？
# 拟合曲面图
# SVM 回归
library(kernlab)
model <- ksvm(z ~ x + y,
  data = volcano_df, C = 1, kernel = "rbfdot"
)

mesh_size <- 1
xrange <- seq(from = min(volcano_df$x), to = max(volcano_df$x), by = mesh_size)
yrange <- seq(from = min(volcano_df$y), to = max(volcano_df$y), by = mesh_size)

final <- expand.grid(x = xrange, y = yrange)
final <- as.matrix(final)
pred <- predict(model, newdata = final)

pred_m <- matrix(pred, ncol = length(xrange), nrow = length(yrange), byrow = TRUE)
# plotly 绘制
plotly::plot_ly(volcano_df, x = ~x, y = ~y, z = ~z) |>
  plotly::add_markers(size = 5) |>
  plotly::add_surface(
    x = xrange, y = yrange, z = pred_m, alpha = 0.65,
    type = "mesh3d", name = "pred_surface"
  )
# rgl 绘制
plot3d(x = volcano_df$x, y = volcano_df$y, z = volcano_df$z, 
       col = volcano_df$color,
       xlab = "南北方向", ylab = "东西方向", zlab = "高度")
# 拟合曲面
persp3d(x = 1:nrow(volcano), y = 1:ncol(volcano), z = t(pred_m), 
...
除了谷歌提供的地图服务，[SRTM](https://srtm.csi.cgiar.org/)（Shuttle Radar Topography Mission，简称 SRTM）提供 90 米精度的海拔数据，对纬度范围在南纬 60 至北纬 60 度的地区。根据台湾省统计局 2022 年 10 月发布的数据显示，台湾目前有人口 2300 多万。下面获取台湾地区的海拔数据和一级行政区划边界数据。

[**geodata** 包](https://github.com/rspatial/geodata) 获取台湾海拔数据，返回 terra 类型的数据对象：SpatRaster 对象。

```{r}
# 下载台湾海拔数据
twn_map_alt <- geodata::elevation_30s(country = "TWN", path = "data/")
```

获取台湾一级行政区数据，它是一个多边形边界地图数据，也返回 terra 类型的数据对象： SpatVector 对象。

```{r}
# 下载台湾一级行政区
twn_map_adm <- geodata::gadm(country = "TWN", level = 1, path = "data/")
```

[**tidyterra** 包](https://github.com/dieghernan/tidyterra) 基于 [**terra** 包](https://github.com/rspatial/terra) 提供一些专门用于栅格数据可视化的图层，扩展了 **ggplot2** 包的功能，如 @fig-tw-elev 所示台湾海拔地形图。
...
ggplot() +
  tidyterra::geom_spatraster(data = twn_map_alt) +
  tidyterra::scale_fill_terrain_c() +
  tidyterra::geom_spatvector(
    data = twn_map_adm,
    fill = NA, colour = "gray40"
  ) +
  theme_minimal() +
