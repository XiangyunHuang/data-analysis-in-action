...
svn log --xml --verbose -r 6:82626 https://svn.r-project.org/R/trunk > data-raw/svn-trunk-log-2022.xml
```
...
其中，文盲人口是指15岁及15岁以上不识字及识字很少人口。文盲率 = 文盲人口 / 15岁以上人口。抚养比 = (0-14岁 + 65岁及以上) / 15-64岁人口数。

```{r}
#| label: fig-china-raise-illiteracy
#| fig-cap: "文盲人口占15岁及以上人口的比重与总抚养比的关系"
#| fig-width: 6
#| fig-height: 4
#| fig-showtext: true

ggplot(data = china_raise_illiteracy, aes(x = `总抚养比`, y = `文盲人口占15岁及以上人口的比重`)) +
  geom_point() +
  theme_classic() +
  labs(x = "总抚养比（%）", y = "文盲人口占15岁及以上人口的比重（%）")
```
...
ggbump 有警告
2: In dplyr::left_join(., data %>% dplyr::select(-x, -y) %>% dplyr::mutate(key = 1) %>%  :
...
[ggpval](https://github.com/s6juncheng/ggpval) 在图上添加检验的 P 值结果，[**ggsignif**](https://github.com/const-ae/ggsignif) 在图上添加显著性注释。[ggstatsplot](https://github.com/IndrajeetPatil/ggstatsplot) [@Indrajeet2021] 可视化统计检验、模型的结果，描述功效变化。
...
library(ggpval)
p_val <- ggplot(PlantGrowth, aes(x = group, y = weight)) +
...
# 模型诊断图
fit <- lm(`Life Exp` ~ Income, data = state_x77)
...
[ggeffects](https://github.com/strengejacke/ggeffects) 描述单个变量的作用
...
mydf <- ggpredict(fit, terms = "Income")
...
继续以 state_x77 数据为例，以贝叶斯线性模型拟合数据，获得参数的后验分布。
...
library(bayesplot)
# 迭代轨迹
p1 <- mcmc_trace(fit_state_x77$draws(c("alpha", "beta[1]")),
  facet_args = list(
...
# 参数的后验分布
p2 <- mcmc_hist(fit_state_x77$draws(c("alpha", "beta[1]")),
  facet_args = list(
    labeller = ggplot2::label_parsed,
    strip.position = "bottom"
  )
