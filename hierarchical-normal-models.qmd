...
`lp__` 后验分布的对数，`pairs` 函数中再添加参数 `log = TRUE` 可获得非负参数取对数后的分布图

```{r}
#| label: fig-parameters-posterior
#| fig-cap: 参数 $\mu$，$\tau$ 和 $\mathrm{lp__}$ 的后验分布图
#| fig-showtext: true
...
模型输出结果中，截距 106.56762，斜率 6.18571。这与 Stan 拟合该模型的输出结果是一致的。Stan 模型中参数 `beta_c` 表示斜率，生成的量 `alpha0` 代表截距。

```{r}
print(rats_fit, c("alpha0", "beta_c", "tau_c"), digits = 3)
```

经过对比 Stan 和 **nlme** 包的拟合结果，可以发现贝叶斯和频率方法的结果是一致的。
