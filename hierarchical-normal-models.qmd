...
```{r}
lapply(eight_schools_sim, mean)
```

计算 $\mu,\tau$ 等参数的分位点

```{r}
lapply(eight_schools_sim, quantile, probs = c(2.5, 25, 50, 75, 97.5) / 100)
```
...
matplot(
  eight_schools_mu_sim, xlab = "Iteration", ylab = expression(mu),
  type = "l", lty = "solid", col = custom_colors
)
abline(h = apply(eight_schools_mu_sim, 2, mean), col = custom_colors)
legend(
  "topleft", legend = paste0("chain:", 1:2), box.col = "white", 
  inset = 0.01, lty = "solid", horiz = TRUE, col = custom_colors
)
```

也可以使用 **rstan** 提供的 `traceplot()` 函数或者 `stan_trace()` 函数绘制参数的迭代轨迹图。
...
可以用 `stan_hist()` 函数或 `stan_dens()` 函数绘制后验分布图。参数 $\mu$ 和 $\tau$ 的直方图、散点图、参数 $\mu$ 的后验概率密度分布图。
...
p3 <- stan_dens(eight_schools_fit, pars = "mu") + 
  labs(x = expression(mu))
library(patchwork)
p1 / (p2 + p3)
```

**bayesplot** 包的函数 `mcmc_pairs()` 以矩阵图展示多个参数的分布，下图展示参数 $\mu$，$\log(\tau)$ 后验分布图。相比于 rstan 包，**bayesplot** 包可视化能力更强，支持对特定的参数做变换。
