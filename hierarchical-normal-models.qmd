...
```{r}
rats_blme <- blme::blmer(weight ~ days + (days | rats), data = rats_data)
summary(rats_blme)
```

## 习题 {#sec-hierarchical-models-exercises}

1.  四个组的重复测量数据，如下表所示，建立线性混合效应模型/分层正态模型分析数据。
    ...
    knitr::kable(dat2, col.names = c("编号", "第1组", "第2组", "第3组", "第4组"))
    ```
    ...
    $y_{ij}$ 表示第 $i$ 组的第 $j$ 个测量值，$\theta_i$ 组的均值。
...
2.  基于 **lme4** 包中学生对课程的评价数据 `InstEval` 建立（广义）线性混合效应模型分析数据。将响应变量（学生评价）视为有序的离散型变量，比较观察两个模型拟合效果。再者，从 Stan 实现的贝叶斯模型来看，感受 Stan 建模的灵活性和扩展性。（**nlme** 包不支持此等交叉随机效应的表达。）
    ...
    ```{r}
    fit_ratings <- lme4::lmer(y ~ 1 + service + (1 | s) + (1 | d) + (1 | dept), data = InstEval)
    summary(fit_ratings)
    ```
