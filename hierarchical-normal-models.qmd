...
# )
# # proper Cauchy prior with scale equal to sqrt(alpha.V)
# prior3 <- list(
#   R = list(
#     V = diag(schools$sigma^2), fix = 1
#   ),
...
(Intercept)     7.802   -2.062   17.656     1100  0.11
```

值得一提的，早年出现的 R 包 **arm** [@Gelman2007] 支持分层模型，这是最常见和重要的贝叶斯模型。**code** [@coda2006] 可以就 **rjags** [@rjags] 调用 JAGS 返回的结果进行各项数据分析。
...
$$
\begin{aligned}
& y_i \sim \mathcal{N}(\theta_i, \sigma_i^2), \quad \theta_i = \mu_i + \tau*\eta_i \\
& \mu \sim \mathcal{N}(0, 5^2), \quad \tau \sim \mathrm{Half\_Cauchy}(0,5)
\end{aligned}
$$
