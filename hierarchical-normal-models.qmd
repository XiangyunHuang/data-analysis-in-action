...
边际似然中包含随机效应对应的潜高斯分布的积分，INLA 包采用集成嵌套拉普拉斯近似积分。INLA 包拟合随机截距和斜率的混合效应模型 [@Wang2018]。

作为一个参照，首先考虑变截距的随机效应模型。
...
library(INLA)
# 变截距
rats_inla0 <- inla(weight ~ 1 + days + f(rats, model = "iid"),
...
输出结果显示，$\beta_0 = 106.568,\beta_1 = 6.186$ 。随机效应部分需要做一下转化，Precision 需要转化为 Variance

```{r}
# 组内 残差方差 标准差
sigmaepsilon <- inla.tmarginal(
  function(x) 1 / sqrt(exp(x)),
  rats_inla0$internal.marginals.hyperpar[[1]]
)
# 组间 随机方差 标准差
sigmaalpha <- inla.tmarginal(
  function(x) 1 / sqrt(exp(x)),
  rats_inla0$internal.marginals.hyperpar[[2]]
)
...
结果显示，组间方差和残差方差依次为 $\sigma_{\alpha} = 13.55, \sigma_{\epsilon} = 8.19$ 。

接下来，考虑截距和斜率同时变化的随机效应模型，假定斜率和截距的联合分布是二元高斯分布。

```{r}
rats_data$slopeid <- rats_data$rats + N
# 变截距、变斜率
rats_inla <- inla(
  weight ~ 1 + days + f(rats, model = "iid2d", n = 2*N) + f(slopeid, days, copy = "rats"),
...
固定效应部分的参数估计结果一样，但估计量的标准差不一样，截距项变小了，斜率项变大了。
...
-   `Precision for rats (component 1)` 对应于随机效应部分的截距的 SD
-   `Precision for rats (component 2)` 对应于随机效应部分的斜率的 SD
-   `Rho1:2 for rats` 对应于随机效应部分的截距和斜率的 SD 的相关系数

从输出结果来看，`Fixed effects:` 部分与前面的输出完全一样。贝叶斯推断给出的是参数的后验分布，所以，不仅是后验点估计，同时给出几个后验分布的分位点，置信水平 95% 的置信区间为 $[103.559, 109.577]$ 。从结果中提取，截距项 `(Intercept)` 的后验边际分布
...
# 固定效应中的截距项
beta0 <- rats_inla$marginals.fixed[[1]]
library(ggplot2)
ggplot(data.frame(inla.smarginal(beta0)), aes(x, y)) +
  geom_line() +
  theme_bw()
```
...
```{r}
quant <- inla.qmarginal(0.05, beta0)
quant
```

下面用图表示 $P(X < 104.0454) = 0.05$ ，随机变量 $X$ 小于 0.05 分位点的概率。
...
ggplot(data.frame(inla.smarginal(beta0)), aes(x, y)) +
  geom_line() +
  geom_area(data = subset(data.frame(inla.smarginal(beta0)), x < quant), fill = "gray") +
  theme_bw()
```
...
rats_inla$marginals.hyperpar$`Precision for rats (component 1)`)
```
