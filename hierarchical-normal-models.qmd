...
从迭代抽样数据获得与 `print(fit)` 一样的结果。以便后续对原始采样数据做任意的进一步分析。rstan 包扩展泛型函数 `summary()` 以支持对 stanfit 数据对象汇总。输出分链条和合并链条的汇总结果

```{r}
# 汇总结果
print(summary(eight_schools_fit)$summary, digits = 2)
```
...
示例数据来自 Gelfand et al (1990) ，每隔一周记录 30 只小鼠的重量，一共进行了 5 周。第一次记录是小鼠第 8 天的时候，第二次测量记录是第 15 天的时候，一直持续到第 36 天。下面在 R 环境中准备数据。
...
y_{it} &\sim \mathcal{N}(\alpha_i + \beta_i * (x_t - \bar{x}), \tau_c) \\
& i = 1,2,\ldots,N \quad t = 1,2,\ldots,T
\end{aligned}
$$

其中， $\alpha_c,\beta_c,\tau_c,\tau_{\alpha},\tau_{\beta}$ 为无信息先验，$N = 30$ 和 $T = 5$ 分别表示实验中的小鼠数量和测量次数，下面编码、编译、采样和拟合模型。
...
Stan 模型中参数 `beta_c` 表示斜率，生成的量 `alpha0` 代表截距。
...
模型输出结果中，截距 106.56762，斜率 6.18571。这与 Stan 拟合该模型的输出结果是一致的。经过对比 Stan 和 **nlme** 包的拟合结果，可以发现贝叶斯和频率方法的结果是一致的。

::: callout-important
当采用 **lme4** 包拟合数据的时候，发现参数取值落在参数空间的边界上，除此之外，输出结果与 **nlme** 包的结果完全一致。

```{r}
mod_rats_lme4 <- lme4::lmer(weight ~ days + (1 | rats), rats_data)
sumary(mod_rats_lme4)
```
:::
