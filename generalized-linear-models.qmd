...
接下来，编译 Stan 代码，准备参数初值，配置采样的参数。首先加载 **cmdstanr** 包，设置 4 条迭代链，给每条链设置相同的参数初始值。代码编译后，生成一个模型对象 `mod_gaussian`，接着，调用方法 `sample()` ，传递迭代初值 `init`，初始化阶段的迭代次数 `iter_warmup` ，采样阶段的迭代次数 `iter_sampling`，采样的链条数 `chains` 及并行时 分配的 CPU 核心数 `parallel_chains` ，随机数种子 `seed` 。
...
默认情况下，采样过程中会输出一些信息，以上是 4 条链并行采样的过程，给出百分比进度及时间消耗。采样完成后，调用方法 `summary()` 汇总和展示采样结果。

```{r}
fit_gaussian$summary()
```

输出模型中各个参数的后验分布的一些统计量，如均值（mean）、中位数（median）、标准差（sd），0.05 分位点（q5），0.95 分位点（q95）等。此外，还有 \`lp\_\_\` 后验对数概率密度值，每个模型都会有该值。`summary()` 方法有一些参数可以控制数字的显示方式和精度。下面展示的是保留 4 位有效数字的结果。
...
一般地，不妨设 $x_1,x_2,\cdots,x_n$ 是来自正态总体 $\mathcal{N}(\mu,1)$ 的一个样本。则正态分布的概率密度函数 $f(x)$ 的对数如下：

$$
\log f(x) = \log \frac{1}{\sqrt{2\pi}} - \frac{(x - \mu)^2}{2}
$$

经过计算，我们知道参数 $\mu$ 是一个非常接近 0 的数，不妨将 $\mu = 0$ 代入计算

```{r}
sum(dnorm(x = y, mean = 0, sd = 1, log = TRUE))
```

如果去掉常数项计算概率密度函数值的对数和。
...
这就比较接近原 `lp__` 的值了。在知道了 target 的含义后，以概率密度函数的对数 `normal_lpdf` 替代抽样语句，下面这段代码就不难理解了。**`normal_lpdf`** 是 Stan 内置的函数，输入值为概率密度值 `y` 、位置参数 `mu` 和尺度参数 `sigma`，返回值为 `real` 实数。

`real` **`normal_lpdf`**`(reals y | reals mu, reals sigma)`

```{stan output.var="mod_gaussian_lpdf"}
transformed data {
  vector[7] y = [-0.5657,  0.3210,  0.1253,  1.3532, -1.2877,  0.3226,  0.2669]';
  // vector[7] y = [-3, -2, -1, 0, 1, 2, 3]';
}
parameters {
  real mu;
...
可以看到，将随机变量换成它的概率密度值，不影响对模型参数的推断。数值上的差异来源于数值精度的表示。
