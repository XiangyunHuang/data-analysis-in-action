...
我们知道参数 $\mu$ 是一个非常接近 0 的数，不妨将 $\mu = 0$ 代入计算。

```{r}
sum(dnorm(x = y, mean = 0, sd = 1, log = TRUE))
```

对比去掉常数项后，概率密度函数值的对数和。
...
这就比较接近原 `lp__` 的值了，所以，`lp__` 表示后验概率密度函数的对数（扔掉了与参数无关的常数项）。

下面这个示例稍微修改已知数据，之前已知的是概率分布中随机变量的取值，现在已知的是随机变量对应的概率密度函数值的对数，以概率密度函数的对数 `normal_lpdf` 替代抽样语句，下面这段代码就不难理解了。**`normal_lpdf`** 是 Stan 内置的函数，输入值为概率密度值 `y` 、位置参数 `mu` 和尺度参数 `sigma`，返回值为 `real` 实数。
