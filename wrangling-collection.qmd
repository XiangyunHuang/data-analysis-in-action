...
从中央到地方的各级政府每年都发布大量的公告，这些公告里包含大量的数据信息。比如财政部官网每年发布各类财政数据，各项收入、支出、债务以及转移支付等，下面从国务院关于中央决算的报告里，获取国债余额限额数据。

```{r}
library(rvest)
# 国务院关于中央决算的报告 2022-2024 年
...
# 下载网页
web_pages <- lapply(links_df$link, read_html)
# 提取文本内容
...
```{r}
# 筛选国债余额限额数据
x <- lapply(web_texts, stringr::str_extract, pattern = "(国债余额限额)(\\d+.*?)亿元")
lapply(x, function(x) {
  x[!is.na(x)]
})
```
