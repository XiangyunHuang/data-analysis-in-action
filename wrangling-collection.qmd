...
基于开放数据库连接（Open Database Connectivity，简称 ODBC）标准，**odbc** 包提供了一种统一的连接各类数据库管理系统（Database Management System，简称 DBMS）的 R 接口。以连接前面的 SQLite 数据库为例，介绍 odbc 包的使用。首先[安装 ODBC 驱动](https://odbc.r-dbi.org/articles/setup.html)，接着，调用 **odbc** 包连接和使用数据库。
...
| Driver          = /opt/homebrew/Cellar/sqliteodbc/0.99991/lib/libsqlite3odbc.dylib | Driver          = /usr/lib/x86_64-linux-gnu/odbc/libsqlite3odbc.so |
| ```                                                                                | ```                                                                |
+------------------------------------------------------------------------------------+--------------------------------------------------------------------+

: 在文件 `odbc.ini` 配置数据库信息
