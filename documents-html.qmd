...
行内公式一般用来写数学符号，行间公式一般用来排版数学公式，特别是多行公式。

$$\mathbf{y} = X\boldsymbol{\beta} + \boldsymbol{\epsilon}$$

排版行间公式又很多不同的 LaTeX 环境，最常见的又有两种，一种是多个公式逐行排，一种是长公式折行，常常要求对齐。线性模型的两种表示方式，一种是矩阵向量式，一种是数据结构式，见 @eq-doc-lm 。
...
在行间公式中，使用 `split` 环境排版需要拆成多行的一个长公式。线性模型回归系数的最小二乘估计的方差的计算过程，见 @eq-doc-beta 。
