...
ggplot(data = usa_mortality, aes(x = Year, y = Age, fill = Male)) +
  ...
  coord_cartesian(xlim = c(1936, 2017)) +
  theme_classic() +
...
## 展示调查研究中的用户态度 {#sec-likert}

比较 ggplot2 条形图和 likert 条形图的区别，

在市场调查，问卷调查，App 用户体验反馈等方面应用十分广泛。

1932 年 Rensis Likert  [@Likert1932]

一个非常典型的 Likert Items 是 

1. Strongly disagree （强烈不同意） 
1. Disagree（不同意）
1. Neither agree nor disagree（既不表示同意也不表示不同意，没态度）
1. Agree（同意）
1. Strongly agree（强烈同意）

Jason M. Bryer 开发了一个 R 包[likert](https://github.com/jbryer/likert)，特别适合调查研究数据可视化，将研究对象的态度以直观有效的方式展示出来。
