...
`power.t.test()` 计算单样本或两样本的 t 检验的功效，或者根据功效计算参数，如样本量。t 检验的功效随 Cohen's d 的变化情况如下图所示。
...
可见 **pwr** 包的函数 `pwr.t.test()` 和 Base R 内置的函数 `power.t.test()` 在功效的计算上结果一样。下面以 sleep 数据集为例计算功效。
