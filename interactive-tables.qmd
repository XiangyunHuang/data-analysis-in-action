...
```{r}
datatable(head(iris), caption = "鸢尾花数据集（部分）")
```
...
```{r}
datatable(head(iris), caption = htmltools::tags$caption(
  style = 'caption-side: bottom; text-align: left;', '注：鸢尾花数据集（部分）。')
  )
```

### 水平滚动 {#sec-table-scrollx}

当表格中的列比较多时，水平滚动就有用武之地了。
...
当表格中的行比较多时，垂直滚动可以发挥作用。各个参数名来自对应的 JS 库，详见说明 [scrollY](https://datatables.net/reference/option/scrollY) 。

```{r}
datatable(iris, options = list(scrollX = TRUE, scrollY = 300, scrollCollapse = TRUE, paging = FALSE))
```

### 数据分页 {#sec-table-pagination}

当表格中的行比较多时，将表格分页也是一种方法，而且是默认的一种办法。
...
```{r}
datatable(iris, options = list(pageLength = 8))
```

### 适应宽度 {#sec-table-autofill}

```{r}
datatable(iris, options = list(autofill = TRUE, scrollX = TRUE, scrollY = 300, scrollCollapse = TRUE, paging = FALSE))
```
