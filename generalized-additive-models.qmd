...
经过对 brms 包生成的模型代码的分析，发现它采用潜变量高斯过程（latent variable GP）模型，这也是采样效率低下的一个关键因素。接下来，分别手动实现低秩和基样条两种方法近似边际高斯过程（marginal likelihood GP），用 Stan 编码模型。代码文件分别是`rongelap_poisson_lr.stan` 和 `rongelap_poisson_splines.stan` 。
