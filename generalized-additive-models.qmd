...
```{r}
# 根据背景值将各个重金属浓度列进行转化
city_data <- readRDS(file = "data/cumcm2011A.rds")
library(sf)
city_data <- st_as_sf(city_data, coords = c("x(m)", "y(m)"), dim = "XY")
city_data
```
...
mgcv 包用于基于惩罚似然的多元模型平滑参数估计和选择 [@wood2016]。
