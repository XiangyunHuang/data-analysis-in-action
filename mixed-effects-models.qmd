...
1.  **MASS** 包的地形数据集 topo 为例，高斯过程回归模型
    ...
    topo_grid_df <- expand.grid(
      x = seq(from = 1, to = 6.5, by = 0.25),
      y = seq(from = 1, to = 6.5, by = 0.25)
    )
    ...
    mod_topo_gaussian <- cmdstan_model(
      stan_file = "code/topo_gaussian_pred.stan",
    ...
    library(stars)
    topo_grid_stars <- st_rasterize(topo_grid_sf, nx = 23, ny = 23)

    library(ggplot2)
    ggplot() +
      geom_stars(data = topo_grid_stars, aes(fill = ypred), na.action = na.omit) +
      scale_fill_viridis_c(option = "C") +
      theme_bw()
    ```
