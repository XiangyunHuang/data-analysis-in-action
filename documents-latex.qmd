...
cat(readLines("code/rmarkdown-chinese.Rmd"), sep = "\n")
```
...
cat(readLines("code/quarto-chinese.qmd"), sep = "\n")
```
...
`@tbl-quarto-table` 插入图片 @tbl-quarto-table ，`@fig-quarto-figure` 插入图片 @fig-quarto-figure 。引用表格，标识符的前缀必须是 tbl，引用插图，标识符的前缀必须是 fig，以连字符连接。对比 LaTeX 文档中的图、表引用 `\ref{fig:figure}` ，`@` 对应于 `\ref` 。值得注意，在 LaTeX 文档中，对标识符的命名没有要求，但通常为了区分引用的内容，会加上不同的前缀。
