...
### Likert Scale 图 {#sec-likert-scale}

展示调查研究中的用户态度。量表在市场调查，问卷调查，App 用户体验反馈等方面应用十分广泛，已经成为调查研究中的黄金标准。量表由心理学家 Rensis Likert 于 1932 年提出 [@Likert1932]，[Likert Scale](https://en.wikipedia.org/wiki/Likert_scale) 就是以他的名字命名的。
...
MathAnxiety_df <- reshape(data = MathAnxiety, 
  ...
  v.names = "Numbers", 
  idvar = "Item", 
  new.row.names = 1:(5 * 14),
  direction = "long"
 )

ggplot(data = MathAnxiety_df, aes(x = Numbers, y = Item, fill = Attitude)) +
  geom_col(position = "fill") +
...
**ggstats** 包的函数 `gglikert()` 适合对明细的调查数据绘图，而 **likert** 包的函数 `likert()` 适合对聚合的调查数据绘图。
