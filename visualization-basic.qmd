...
**ggmosaic** 包提供图层 `geom_mosaic()` 绘制马赛克图。Base R 的函数 `plot()` 是泛型函数，**lattice** 包的 `xyplot()` 也是，以绘制条件图为例，Base R 提供函数 `coplot()`，**lattice** 包也支持公式语法，以竖线 `|` 分割主变量和条件变量，右侧代表条件变量，比如 `~ x | y`。
...
library(lattice)
densityplot(~ Sepal.Length | Species, data = iris, layout = c(3, 1))
library(ggplot2)
...
|**latticeExtra**  |  `rootogram`  | Tukey 悬挂根图 | 描述数据分布 |
|**latticeExtra**  |  `horizonplot`  | 水平线图   | 描述数据趋势 |
