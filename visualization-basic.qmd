...
**ragg** 包 (<https://github.com/r-lib/ragg>) 无需手动配置字体，只要是系统已经安装的字体，在 **ggplot2** 绘图时，将字体名称传递给 `family` 即可。 [**emojifont**](https://github.com/GuangchuangYu/emojifont) 包将 [Emoji 字体](https://github.com/ikatyang/emoji-cheat-sheet)引入 **ggplot2** 绘图，如 @fig-emoji 所示。
...
ggplot(dat, aes(x = fruits_emo, y = weight)) +
  ...
  geom_text(aes(label = fruits_emo),
    size = 12, vjust = -0.5, family = "EmojiOne"
  ) +
  theme_classic() +
...
ggplot2 包内置了一些的数学公式解析和表达能力，[latex2exp](https://github.com/stefano-meschiari/latex2exp) 包和[tikzDevice](https://github.com/daqana/tikzDevice)包大大扩展了数学公式的处理能力，相比于 latex2exp 包，tikzDevice 调用 LaTeX 编译引擎处理数学公式，渲染的公式效果更加精美。

形状参数分别为 $a$ 和 $b$ 的贝塔分布的概率密度函数如下：

$$
f(x) = \frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}x^{a-1}(1-x)^{b-1}, \quad a>0,b>0,0\leq x \leq 1
$$

@fig-beta 中红线对应贝塔分布 $B(3,0.9)$ 而蓝线对应贝塔分布 $B(3,9)$
...
ggplot() +
  geom_function(
    ...
    aes(colour = "dbeta(3,0.9)"), linewidth = 1.2
  ) +
  geom_function(
    fun = dbeta, args = list(shape1 = 3, shape2 = 9),
    aes(colour = "dbeta(3,9)"), linewidth = 1.2
  ) +
  theme_classic() +
  scale_color_manual(values = c("#E41A1C", "#377EB8")) +
  labs(
    x = expression(x), y = expression(f(x)), color = "分布",
