...
[**ggChernoff**](https://github.com/Selbosh/ggChernoff) 包受切尔诺夫 Chernoff 脸表达多元数据的启发 [@Chernoff1973]，借助 Emoji 表情表达数据指标，比如从低收入到高收入的连续型数据，从非常喜欢到非常不喜欢的有序分类型数据，用表情表达此类数据，显得非常形象。下 @fig-chernoff 以收入水平分类，在每个分类中，将预期寿命与表情结合，越长寿越开心。
