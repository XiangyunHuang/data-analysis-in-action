...
[**ggChernoff**](https://github.com/Selbosh/ggChernoff) 包受 Chernoff 脸表达多元数据的启发 [@Chernoff1973]，借助 Emoji 表情表达数据指标，比如从低收入到高收入的连续型数据，从非常喜欢到非常不喜欢的有序分类型数据，用表情表达此类数据，显得非常形象。下 @fig-chernoff 以收入水平分类，在每个分类中，将预期寿命与表情结合，越长寿越开心。
...
ggplot(gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_chernoff(aes(smile = smile, fill = income_level), alpha = 0.75, 
                show.legend = c(smile = FALSE, fill = TRUE)) +
  scale_x_log10(labels = scales::label_log()) +
  scale_fill_brewer(palette = "Spectral") +
  scale_smile_continuous(midpoint = .5) +
  theme_classic() +
