...
经验累积分布用阶梯图表示，不同种类的鸢尾花，花瓣长度的分布明显不同。
...
ecdfplot(~ Petal.Length | Species, data = iris, scales = "free", xlab = "花瓣长度")
```
...
函数 `panel.ellipse()` 来自 **latticeExtra** 包
...
本章主要介绍 **lattice** 包和 **latticeExtra** 包

| R 包             | 函数                 | 含义   | 作用         |
|------------------|----------------------|--------|--------------|
| **lattice**      | `xyplot()`           | 散点图 | 描述关系     |
| **lattice**      | `bwplot()`           | 箱线图 | 描述分布     |
| **lattice**      | `barchart()`         | 柱形图 | 描述对比     |
| **lattice**      | `levelplot()`        | 轮廓图 | 描述趋势     |
| **lattice**      | `wireframe()`        | 透视图 | 描述趋势     |
| **latticeExtra** | `segplot()`          | 点线图 | 描述不确定性 |
| **latticeExtra** | `panel.smoother()`   | 曲线图 | 描述趋势     |
| **latticeExtra** | `panel.2dsmoother()` | 轮廓图 | 描述趋势     |
| **latticeExtra** | `ecdfplot()`         | 阶梯图 | 描述分布     |
| **latticeExtra** | `panel.ellipse()`    | 聚类图 | 描述分布     |
| **latticeExtra** | `mapplot()`          | 面量图 | 描述分布     |
