...
除了通过 space 参数设置图例的位置，还可以通过坐标设置图例的位置，比如下 @fig-lattice-xyplot-key-2 中设置图例的位置坐标为 `x = 0, y = 0.95` 使得图例位于图的左上角。图例坐标的参考点是原点 `x = 0, y = 0` 就是左下角的位置，而右上角的位置为 `x = 1, y = 1` 。
...
xyplot(
  x = Sepal.Length ~ Petal.Length, groups = Species, data = iris,
...
xyplot(
  ...
  scales = "free", grid = TRUE, type = c("p", "r"),
  xlab = "萼片长度", ylab = "花瓣长度", 
  auto.key = list(x = 0, y = 0.95, corner = c(0, 1)),
  par.settings = list(
    superpose.symbol = list(pch = 16),
    superpose.line = list(lwd = 2, lty = 3)
  )
...
```{r}
#| label: fig-lattice-par-settings
...
xyplot(Sepal.Length ~ Petal.Length,
  ...
  par.settings = list(
    superpose.symbol = list(pch = 16),
    superpose.line = list(lwd = 3, lty = 3)
  ),
  panel = function(x, y, ...) {
    panel.xyplot(x, y, ...)
    panel.ellipse(x, y, ...)
