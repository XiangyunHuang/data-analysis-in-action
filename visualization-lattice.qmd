...
xyplot(x = Sepal.Length ~ Petal.Length, groups = Species, scales = "free", 
...
library(splines)
library(mgcv)
xyplot(dist ~ speed, data = cars, scales = "free", xlab = "速度", ylab = "距离",
  panel = function(x, y, ...) {
    panel.xyplot(x, y, ...)
    panel.smoother(y ~ x,
...
levelplot(depth ~ long + lat | Magnitude,
  ...
  layout = c(2, 2), prepanel = prepanel.default.xyplot
)
```
...
levelplot(volcano, col.regions = hcl.colors, useRaster = TRUE,
  # 去掉图形上、右边多余的刻度线
  scales = list(
