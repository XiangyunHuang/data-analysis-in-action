...
# curve(expr = fn, from = -2, to = 5)
library(ggplot2)
ggplot() +
  geom_function(fun = fn, xlim = c(-2, 5), linewidth = 1) +
...
$$
\begin{aligned}
\min z = & ~\sin\Big((zx-0.5)^2 + 2xy^2 - \frac{z}{10} \Big)\cdot \\
&~\exp\Big(-\Big( \big(x-0.5 -\exp(-y+z)\big)^2 + y^2 - \frac{z}{5} + 3 \Big)\Big)
\end{aligned}
$$

其中， $x \in [-1,7],y \in [-2,2]$ 。

对于隐函数 $f(x,y,z)=0$ ，常规的做法是先计算隐函数的偏导数，并令偏导数为 0，再求解非线性方程组，得到各个驻点，最后，将驻点代入原方程，比较驻点处函数值，根据优化目标选择最大或最小值。
