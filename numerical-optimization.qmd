...
1.  比较全面地展示各类优化问题的 R 语言实现，其覆盖面之广，远超市面上同类 R 语言书籍。从线性优化到凸优化（包含凸二次优化和凸锥优化），从简单整数优化到混合整数非线性规划，再到一般的非线性优化，触及最新最前沿的热门话题。

2.  对每类优化问题都给出示例及 R 语言实现，涉及近 10 个各类优化器。大量参照 Lingo 和 1stOpt 等国内外商业优化建模软件的官方示例，给出 R 语言实现，部分示例还和 Octave 对比，确认 R 语言求解器的效果可以达到开源和商业软件的水平。

3.  对于 R 语言社区难以求解的复杂优化问题（比如混合整数非线性规划），通过连接 AMPL 软件，调用开源的优化求解器求解，对这些比较复杂的优化问题，都给出了开源替代方案，并总结了实战经验。
:::
...
-   **scs** 包可解大部分常见的凸锥规划问题，但对于含整数目标变量的规划问题，无法求解，**ECOSolveR** 包可以。
-   对于混合整数二次规划和混合整数非线性规划问题，借助 **rAMPL** 包调用开源的求解器求解。
-   对于复杂的非线性规划问题，因其具有非凸、多模态等特点，求解非常困难，引入 **GA** 包，采用随机优化算法求解，效果达到商业软件水平。
...
**scs** 包不能求解此类规划问题，下面调用 **ECOSolveR** 包求解。
...
目前，**ROI** 包支持的开源求解器都不能处理 MIQP 问题。**ECOSolveR** 包可以求解凸二阶锥规划，部分变量可以是整数。因此，先将凸二次规划转化为凸锥规划问题，再连接 **ECOSolveR** 包提供 `ecos` 求解器，最后，调 `ecos` 求解器求解。
