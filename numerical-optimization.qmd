...
## 锥优化 {#sec-cone-optimization}

什么是锥？锥的数学表示和几何表示。

锥是定义在半正定的矩阵之上的。

$n$ 阶矩阵 $A$ 是对称且半正定的，我们将这样的矩阵构成的集合记为 $\mathcal{K}_{+}^n$ 。
...
$$
\begin{aligned}
\min \quad &\bm{a}^{\top}\bm{x} \\
\text{s.t.} \quad & A\bm{x} + \bm{k} = \bm{b} \\
& \bm{k} \in \mathcal{K}
\end{aligned}
$$

下面举个例子说明，其中，含参对称矩阵 $A(m_1,m_2,m_3)$ 如下：
...
当 $m_3 = 0$ 时，集合 $K$ 的边界表示平面上的一个单位圆，当 $m_3 \in [-1, 1]$ ，集合 $K$ 的边界表示一个椭圆。设置 $m_1 = 2/3, m_2 = 3/3, m_3 = 2/3$ ，为了获得一个直观的几何表示，画图将集合 $K$ 的边界绘制出来。

```{r}
fun <- function(x) {
  (2/3)^2 + (3/3)^2 + x^2 - 2 * (2/3) * (3/3) * x - 1
}
fun(2)
A <- toeplitz(x = 1:3 / 3)
det(A)
...
如果矩阵 $A$ 是半正定的，记为 $A \succeq 0$ ，如果矩阵 $A$ 是正定的，记为 $A \succ 0$ 。记 $n$ 阶实对称矩阵的集合为 $\mathcal{S}^{n}$ 。
...
考虑二维的情形，绘制三维曲面图像，见 @fig-valley-persp 。
...
某人要去美国 10 个城市旅行，分别是亚特兰大 Atlanta、芝加哥 Chicago、丹佛 Denver 、休斯顿 Houston、洛杉矶 Los Angeles、迈阿密 Miami、纽约 New York、旧金山 San Francisco、 西雅图 Seattle、华盛顿特区 Washington DC。10 个城市的分布如 @fig-tsp-problem 所示。从亚特兰大出发，最后回到亚特兰大，如何规划旅行线路使得总行程最短？行程最短的路径是什么？
...
求解结果对应的旅行方案，如 @fig-tsp-solution 所示，依次走过的城市是：洛杉矶、休斯顿、迈阿密、亚特兰大、华盛顿特区、纽约、芝加哥、丹佛、西雅图、旧金山。
...
```{r}
# 理解数据，重新搜集和制作数据替代 LPP2005.RET
# data(LPP2005.RET, package = "fPortfolio")
library(timeDate)
library(timeSeries)
LPP2005.RET <- readRDS(file = "data/LPP2005.RET.rds")
...
求解器 `nloptr.slsqp` 需要给初值，而求解器 `quadprog` 不需要给初值。下面使用 quadprog 来求解组合优化问题。
...
给定风险约束 $\sigma$ 回报最大的组合

```{r}
sigma <- sqrt(t(w) %*% cov(lppData) %*% w)
zero_mat <- slam::simple_triplet_zero_matrix(ncol(lppData))
# 目标函数
foo <- Q_objective(Q = zero_mat, L = colMeans(lppData))
# 二次约束
maxret_constr <- Q_constraint(
  Q = list(cov(lppData), NULL),
  L = rbind(
    rep(0, ncol(lppData)),
    rep(1, ncol(lppData))
  ),
 dir = c("<=", "<="), rhs = c(sigma^2, 1)
)
# 目标规划
...
`quadprog` 求解器不能求解该问题，尝试求解器 `nloptr.slsqp`

```{r}
#| eval: false
# 还需要初始值
sol <- ROI_solve(op, solver = "nloptr.slsqp")
w <- sol$solution
round(w, 4)
w %*% colMeans(lppData)
```
...
Y \sim \left\{
\begin{array}{l}
\mathrm{Poisson}(\lambda_1), & \text{当} ~ X = 1 ~ \text{时},\\
\mathrm{Poisson}(\lambda_2), & \text{当} ~ X = 0 ~ \text{时}.
\end{array} \right.
\end{array}
$$
...
$$
\ell(p) = \sum_{i=0}^{n}y_i \log\big(p\times \exp(-\lambda_1) \times\frac{\lambda_1^{x_i}}{x_i!} + (1 - p)\times \exp(-\lambda_2) \times\frac{\lambda_2 ^{x_i}}{x_i!} \big)
$$

下表数据来自 1947 年 Walter Schilling 发表在 JASA 的一篇文章 [@Schilling1947]。连续三年搜集伦敦泰晤士报刊登的死亡告示，记录每天 80 岁及以上女性死亡人数，经过汇总统计，发现三年里，一天中死亡 6 人的告示共出现有 27 次。
...
### 置信区间估计 {#sec-binomial-confidence-interval}

一元函数最优化问题和求根问题是相关的。在统计应用中，二项分布的比例参数的置信区间估计涉及求根，伽马分布的参数估计涉及求根。
...
其中，$\Gamma(\cdot)$ 表示伽马函数，下 @fig-dgamma 展示两个伽马分布的概率密度函数，形状参数分别为 5 和 9，尺度参数均为 1，即伽马分布 $f(x; 5, 1)$ 和 $f(x; 9, 1)$ 。
