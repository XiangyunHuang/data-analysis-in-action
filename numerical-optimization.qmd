...
-   `maximum` ：目标函数需要求极大还是极小，逻辑值 `TRUE` 或 `FALSE`。
...
## 凸优化 {#sec-convex-optimization}

### 严格凸优化

### 非严格凸优化

### 凸锥优化 {#sec-convex-cone-optimization}
...
\text{s.t.} \quad & A\bm{x} + s = b \\
& s \in K,
\end{aligned}
$$

其中，含参对称矩阵 $A(x_1,x_2,x_3)$ 如下
...
$$
S = \{ (x_1,x_2,x_3) \in \mathbb{R}^3 \mid A(x_1,x_2,x_3) \in \mathcal{S}_{+}^3 \},
$$

$\mathcal{S}$ 是有界的半正定的。

$$
\det(A(x_1,x_2,x_3)) = - (x_1^2 + x_2^2 + x_3^2 -2x_1 x_2 x_3 -1) 
$$

$S$ 的边界可表示为

$$
x_1^2 + x_2^2 + x_3^2 -2x_1 x_2 x_3 = 1
$$ 或等价地表示为
...
\left[\begin{array}{c}
x_1\\x_2
\end{array}\right] = 1 - x_3^2.
\end{split}
$$

当 $x_3 = 0$ 表示平面上的一个单位圆，当 $x_3 \in [-1, 1]$ ，表示一个椭圆。直观表示。

设置 $x_1 = 2, x_2 = 3, x_3 = 2$

```{r}
fun <- function(x){
  2^2 + 3^2 + x^2 - 2*2*3*x - 1
}
fun(2)
(A = toeplitz(x = 1:3))
det(A)
...
空间高斯过程协方差矩阵参数的剖面极大似然估计。关于协方差矩阵的参数的似然函数是非凸的，多模态的。详细描述见 Brian Ripley 早年论文 [@Ripley1987]
...
其中，$\bm{\beta}$ 是一个 $p\times 1$ 维列向量，随机过程 $S(x)$ 是均值为零，协方差为 $\Sigma_{\bm{\theta}}$ 的平稳高斯过程，协方差矩阵 $\Sigma_{\bm{\theta}}$ 的元素如下：

$$
\mathsf{Cov}\{S(x_i), S(x_j)\} = \sigma^2 \exp(-\|x_i - x_j\| / \phi)
$$

其中， $\bm{\theta} = (\sigma^2,\phi)$ 表示与协方差矩阵相关的参数，随机过程 $S(x)$ 服从多元正态分布 $\mathrm{MVN}(\bm{0},\Sigma_{\bm{\theta}})$ ，则 $\bm{y}(x)$ 也服从多元正态分布 $\mathrm{MVN}(D\bm{\beta},\Sigma_{\bm{\theta}})$ 。

参数 $\bm{\beta}$ 的广义最小二乘估计 $\hat{\bm{\beta}}(\bm{\theta}) = (D^{\top}\Sigma_{\bm{\theta}}^{-1}D)^{-1} D^{\top}\Sigma_{\bm{\theta}}^{-1}\bm{y}$

关于参数 $\bm{\theta}$ 的对数似然函数如下

$$
\log \mathcal{L}(\bm{\theta}) = -\frac{n}{2}\log (2\pi) - \frac{1}{2}\log (\det \Sigma_{\bm{\theta}}) -\frac{1}{2}\bm{y}^{\top}\Sigma_{\bm{\theta}}^{-1}\big(I - D(D^{\top}\Sigma_{\bm{\theta}}^{-1}D)^{-1}D^{\top}\Sigma_{\bm{\theta}}^{-1}\big)\bm{y}
$$

根据 topo 数据集， $D = \bm{1}$ 是一个 $n \times 1$ 的列向量，$\bm{\beta} = \beta$ 是一个截距项。
...
log_lik <- function(x) {
  ...
  as.vector(-1 / 2 * log(det(Sigma)) - 1 / 2 * t(topo[, "z"]) %*% inv_Sigma %*% (diag(1, n) - D %*% solve(t(D) %*% inv_Sigma %*% D, t(D)) %*% inv_Sigma) %*% topo[, "z"])
}
log_lik(x = c(65, 2))
```

不同的初始值，收敛到不同的位置，目标函数值非常接近。
...
如果初始值靠近局部极值点，则就近收敛到该极值点。比如初值 $(65, 7)$
...
尝试调用全局优化求解器，大大小小的坑都跳过去了，结果还是比较满意的。
...
上图中没有看到许多局部极小值，与作者论文中的图 1 似乎不符。原因是什么？似然函数中涉及到的矩阵运算不精确，应该设计精度更高的运算方式？**lattice** 包绘图引擎无法展示更加细微的差异？

还有一种解释，上图是对的，算法迭代时，对不同的初值，常常收敛到不同的结果，而这些不同的结果都位于岭上不同位置，对应的对数似然值却又几乎一样。
