...
op <- OP(
  ...
  constraints = L_constraint(L = A, dir = rep("<=", 3), rhs = bvec),
  maximum = FALSE
)
op
```
...
在可行域上画出等高线，标记目标解的位置， @fig-quadprog 展示无约束和有约束条件下的解。图中红点表示无约束下的解，黄点表示线性约束下的解。
...
蓝色的点是无约束情况下，目标函数的最小值，而求解器 `nloptr.slsqp` 给出的最优解是红点，令人疑惑。
...
如果矩阵 $A$ 是半正定的，记为 $A \succeq 0$ ，如果矩阵 $A$ 是正定的，记为 $A \succ 0$ 。记 $n$ 阶实对称的集合为 $\mathcal{S}^{n}$ 。
...
某人要去美国 10 个城市旅行，分别是亚特兰大 Atlanta、芝加哥 Chicago、丹佛 Denver 、休斯顿 Houston、洛杉矶 Los Angeles、迈阿密 Miami、纽约 New York、旧金山 San Francisco、 西雅图 Seattle、华盛顿特区 Washington DC。10 个城市的分布如 @fig-us-city-tsp 所示。从亚特兰大出发，最后回到亚特兰大，如何规划旅行线路使得总行程最短？行程最短的路径是什么？

```{r}
#| label: fig-us-city-tsp
#| fig-width: 6
#| fig-height: 3.8
#| fig-showtext: true
#| fig-cap: "10 个城市的分布图"
...
library(TSP)
D_tsp <- as.TSP(D)
tour_sol <- solve_TSP(x = D_tsp, method = "two_opt")
tour_sol
```

途经 10 个城市的最短路程为 7373 。
...
依次走过的城市是：洛杉矶、休斯顿、迈阿密、亚特兰大、华盛顿特区、纽约、芝加哥、丹佛、西雅图、旧金山。

::: exercise
某人要周游美国各州，从纽约出发，走遍 50 个州的行政中心，最后回到纽约。规划旅行线路使得总行程最短。Base R 内置的 R 包 **datasets** 包含美国 50 个州的地理中心数据 `state.center` 。
:::

### 投资组合优化问题 {#sec-markowitz-portfolio-optimization}

Markowitz 投资组合优化问题如下：

### 垃圾邮件分类 {#sec-spam-email-classification}

高斯过程用于分类问题，选用高效且依赖少的 **kernlab** 包求解。
