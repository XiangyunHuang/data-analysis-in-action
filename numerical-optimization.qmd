...
整数优化情况有很多，本文仅考虑以下几类简单情形：

1.  目标函数和约束条件为线性，变量取值都为整数的整数优化。
2.  目标函数和约束条件为线性，变量取值为 0 或 1 的 0-1 整数优化。
3.  目标函数和约束条件为线性，部分变量带有整数约束的混合整数优化。
4.  目标函数为凸二次和约束条件为线性，部分变量带有整数约束的混合整数优化。
...
目前，ROI 支持的开源求解器都不能处理 MIQP 问题，只有 **CVXR** 包和 [**rmpk**](https://github.com/r-opt/rmpk) 包能解 MIQP 问题，更多关于 **CVXR** 包的介绍见[示例库](https://cvxr.rbind.io/examples/)。
...
-   整数线性规划求解器 [cbc](https://github.com/coin-or/Cbc)。
-   整数非线性规划求解器 [Bonmin](https://github.com/coin-or/Bonmin) 和 [Couenne](https://github.com/coin-or/Couenne)。
-   非线性规划求解器 [Ipopt](https://github.com/coin-or/Ipopt)。
...
# 从 AMPL 官网安装 rAMPL 包
install.packages("https://ampl.com/dl/API/rAMPL.tar.gz",
  repos = NULL,
