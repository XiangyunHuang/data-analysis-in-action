...
其中， $\bm{\theta} = (\sigma^2,\phi)$ 表示与协方差矩阵相关的参数，随机过程 $S(x)$ 服从多元正态分布 $\mathrm{MVN}(\bm{0},\Sigma_{\bm{\theta}})$ ，则 $\bm{y}(x)$ 也服从多元正态分布 $\mathrm{MVN}(D\bm{\beta},\Sigma_{\bm{\theta}})$ 。参数 $\bm{\beta}$ 的广义最小二乘估计为 $\hat{\bm{\beta}}(\bm{\theta}) = (D^{\top}\Sigma_{\bm{\theta}}^{-1}D)^{-1} D^{\top}\Sigma_{\bm{\theta}}^{-1}\bm{y}$ ，关于参数 $\bm{\theta}$ 的对数似然函数如下：
...
根据 topo 数据集， $D = \bm{1}$ 是一个 $n \times 1$ 的列向量，$\bm{\beta} = \beta$ 是一个截距项。设置参数初值 $(\sigma,\phi) = (65,2)$ 。用 R 语言编码的似然函数如下：
...
作为验证，下面调用 **nlme** 包的 `gls()` 函数拟合数据，参数估计结果与全局优化求解器的结果比较一致。参数估计结果 $(\sigma, \phi)= (63.93429, 6.121352)$
