...
直观起见，绘制目标函数在区域 $[-50, 50] \times [-50, 50]$ 内的图像，见 @fig-super-function ，可以看到几乎没有变化的梯度，给寻优过程带来很大困难。
...
实际上，该优化问题的最优解在 $(7.999982, 7.999982)$ 取得，目标函数值为 -7.978832。

```{r}
fn(x = c(7.999982, 7.999982))
```

不失一般性，下面考虑 $x_1,x_2 \in [-50,50]$ ，面对如此复杂的函数，调用全局优化器寻优。
