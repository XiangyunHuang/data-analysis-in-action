...
2.  对每类优化问题都给出示例及 R 语言实现，涉及近 10 个各类优化器。大量参考 Lingo 和 1stOpt 等国内外商业优化建模软件的官方示例，部分参考开源软件 Octave （语法大量兼容 Matlab 的科学计算软件）的非线性优化示例，给出 R 语言实现。经过对比，确认 R 语言求解器的效果可以达到同类开源和商业软件的水平。
...
本章介绍五类典型的优化问题及 R 语言求解过程。除了 R 软件内置一些数值优化求解器，R 语言社区有大量的运筹优化方面的函数和 R 包。本文将重点介绍 **ROI** 包 [@ROI2020]，它通过插件包对 20 多个 R 包提供统一的调用方式，相当于一个运筹优化方面的基础设施平台，极大地方便学习和使用。
