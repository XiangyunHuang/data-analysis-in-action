...
目前， R 语言在数据操作方面陆续出现三套工具，最早的是 Base R（1997 年 4月），之后是 **data.table**（2006年4月） 和 **dplyr**（2014 年1月）。下面以 R 软件内置的 iris 数据集为例，介绍三者的常用数据操作。

## 操作工具 {#sec-tools}

-   Base R 指的是 R 软件自带的一套操作数据的工具，只要安装了 R 软件就可使用，各个操作函数分布在基础 R 包中，主要且常见的操作对象是数据框（data.frame）。

-   **data.table** 是一个 R 包，它强化和扩展了 Base R 操作数据的功能，提供一致的链式法则，更高效的功能实现，兼容 Base R 的函数和数据。

-   **dplyr** 是另一个扩展包，它基于一套新的数据操作语法，各个主要函数语义上比较接近 SQL 查询语言，代码易读。

| 操作工具   | 依赖   | 稳定   | 效率 |
|------------|--------|--------|------|
| Base R     | 零依赖 | 很稳定 | 一般 |
| data.table | 少依赖 | 稳定   | 很高 |
| dplyr      | 依赖多 | 一般   | 高   |

: Base R、data.table 和 dplyr {#tbl-data-wrangling}
