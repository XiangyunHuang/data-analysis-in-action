...
```{r}
plot(Sepal.Length ~ Sepal.Width, data = iris, ann = F)
title(main = "Edgar Anderson's Iris Data",
...
text(x = 6, y = 7, labels = "setosa", col = "#EA4335", xpd = T)

mtext(text = "setosa", side = 4, line = 0, outer = T)
mtext(text = "versicolor", side = 4, line = 0, outer = T)
mtext(text = "virginica", side = 4, line = 0, outer = T)
```
...
plot(x = c(2, 6), y = range(iris$Sepal.Length), ann = F, type = "n")
points(Sepal.Length ~ Sepal.Width,
  col = Species, pch = 16, data = iris
)
legend("right", title = "Species",
  legend = unique(iris$Species), box.col = "gray",
  pch = 16, col = c("#EA4335", "#4285f4", "#34A853")
)
```
