...
其中，$\beta,\sigma,\phi,\delta,\Sigma,I$ 的含义同前，$\lambda$ 代表辐射强度，$\mathrm{offsets}$ 代表漂移项，这里是时间段，$\bm{y}$ 表示观测的辐射粒子数，$\mathrm{poisson\_log}$ 表示泊松分布的对数参数化，将频率参数 rate 的对数 $\lambda$ 作为参数，详见 Stan 函数手册中泊松分布的[对数函数表示](https://mc-stan.org/docs/functions-reference/poisson-distribution-log-parameterization.html)。
...
1.  软件安装和配置过程复杂。**rstan** 包内置的 Stan 版本远低于最新发布的 Stan 版本，编译工具，**rstan** [终于要发布新版本了](https://github.com/stan-dev/rstan/issues/1053)。
