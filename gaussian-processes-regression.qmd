...
rongelap_sf$lambda <- as.matrix(lambda1_array[1,1,])
rongelap_sf$yhat <- as.matrix(yhat_array[1,1,])
```
...
```{r}
ypred_array <- fit_rongelap_poisson_pred$draws(variables = "ypred", format = "array")
rongelap_grid_df$ypred <- as.matrix(ypred_array[1,1,])
# 查看预测结果
head(rongelap_grid_df)
# 预测值的分布范围
...
```{r}
subset(rongelap_grid_df, subset = ypred / time > 20)
```
...
p4 <- ggplot() +
  ...
  scale_fill_viridis_c(option = "C", breaks = 2*0:11) +
  # 虚线框
  geom_sf(data = dash_sfp, fill = NA, linewidth = 0.75, lty = 2) +
  theme_void() +
