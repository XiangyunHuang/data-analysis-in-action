...
**ncvreg** 包 [@Breheny2011] 提供额外的两种惩罚类型，分别是 MCP （minimax concave penalty）和 SCAD（smoothly clipped absolute deviation）。
...
```{r}
fit_mcp_cv <- cv.ncvreg(X = Boston[, -14], y = Boston[, "medv"], penalty = "MCP", seed = 20232023)
summary(fit_mcp_cv)
```
...
```{r}
fit_scad_cv <- cv.ncvreg(X = Boston[, -14], y = Boston[, "medv"], penalty = "SCAD", seed = 20232023)
summary(fit_scad_cv)
```
